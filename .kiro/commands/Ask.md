...
1. ✅ **对话提问**：向用户提问以澄清需求或诊断问题
2. ✅ **信息总结**：总结已收集的信息
3. ✅ **创建 PRD 文档**：在 `文档/开发文档/PRD/` 目录创建需求文档
4. ✅ **创建 BUG 文档**：在 `文档/开发文档/BUG/` 目录创建问题分析文档

**严禁以下操作：**

- ❌ **禁止修改任何代码文件**（.kt, .java, .xml, .gradle 等）
- ❌ **禁止创建或修改测试文件**
- ❌ **禁止创建 FD/TDD/TD/CR/DR/IMPL 等其他类型文档**
- ❌ **禁止执行构建、测试等命令**
- ❌ **禁止修改配置文件**

**如果用户要求执行上述禁止操作，请拒绝并说明：**
> "Ask 指令仅用于需求澄清和问题诊断，不执行代码修改或其他文档创建。请使用其他指令完成该操作。"
...
**输出物**：PRD 文档（保存到 `文档/开发文档/PRD/`）

### 场景二：Bug 问题诊断
当用户报告一个 Bug，但信息不足以定位问题时使用。
**输出物**：BUG 文档（保存到 `文档/开发文档/BUG/`）
...
根据澄清/诊断的结果，**仅允许**输出以下文档：

### PRD 需求澄清场景
- **输出**：PRD 文档
- **保存路径**：`文档/开发文档/PRD/PRD-xxxxx-[功能名称]需求.md`
- **编号规则**：查询现有最大编号后递增

### Bug 问题诊断场景
- **输出**：BUG 分析文档
- **保存路径**：`文档/开发文档/BUG/BUG-xxxxx-[问题名称]问题分析.md`
- **编号规则**：查询现有最大编号后递增
...
在对话过程中，如果用户要求：
- "帮我改一下代码" → 拒绝，建议使用其他指令
- "创建一个技术设计文档" → 拒绝，Ask 指令仅输出 PRD/BUG 文档
- "运行一下测试" → 拒绝，Ask 指令不执行命令
- "帮我实现这个功能" → 拒绝，Ask 指令仅用于需求澄清

**正确的做法**：完成需求澄清后，告知用户可以使用其他指令（如 /Research、/CodeReview 等）继续后续工作。
