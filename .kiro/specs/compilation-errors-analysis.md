...
**错误类型**：
- 类型不匹配（Long vs Double）
- when 表达式不完整
- 未解析的引用（abs, averageEffectiveness）
...
**错误类型**：
- 未解析的引用（createDefault, createEnhancedDefault）
- 类型推断失败
- 未解析的属性（replySuggestion, averageDurationMs）
