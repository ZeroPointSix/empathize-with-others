...
### 模块文件统计（基于实际代码架构 - 2025-12-29最新扫描）

| 模块 | 主源码 | 单元测试 | Android测试 | 禁用测试 | 总计 | 测试覆盖率 |
|------|--------|---------|------------|---------|------|-----------|
| **:domain** | 148 | 28 | 0 | 0 | 176 | 18.9% |
| **:data** | 64 | 19 | 4 | 0 | 87 | 35.9% |
| **:presentation** | 245 | 22 | 5 | 0 | 272 | 10.0% |
| **:app** | 22 | 140 | 25 | 5 | 192 | 92.3% |
| **总计** | **479** | **209** | **34** | **5** | **807** | **50.9%** |

**文件构成说明**：
- 主源码：479个文件（domain:148, data:64, presentation:245, app:22）
- 单元测试：209个文件（domain:28, data:19, presentation:22, app:140）
- Android测试：34个文件（data:4, presentation:5, app:25）
- 禁用测试：5个文件（app模块）
- app模块测试覆盖率异常高是因为包含了其他模块的集成测试

**各模块详细构成**：
- **domain**: 66个模型 + 13个Repository接口 + 38个UseCase + 2个Service + 29个Util = 148
- **data**: 7个Entity + 7个DAO + 13个Repository实现 + 6个Parser + 7个DI + 其他24个 = 64
- **presentation**: 187个UI组件 + 19个ViewModel + 4个Navigation + 14个Theme + 其他21个 = 245
- **app**: 11个DI + 11个其他 = 22
