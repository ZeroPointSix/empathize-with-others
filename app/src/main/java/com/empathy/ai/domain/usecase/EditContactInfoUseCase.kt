...
class EditContactInfoUseCase @Inject constructor(
    ...
    suspend fun editName(
        ...
        try {
            ...
            if (rowsAffected > 0) {
                DebugLogger.d(TAG, "联系人姓名编辑成功: contactId=$contactId")
                Result.success(EditResult.Success)
            } else {
                DebugLogger.w(TAG, "联系人姓名编辑失败: 未找到记录 contactId=$contactId")
                Result.success(EditResult.NotFound)
            }
        } catch (e: Exception) {
            DebugLogger.e(TAG, "联系人姓名编辑失败", e)
    ...
    suspend fun editGoal(
        ...
        try {
            ...
            if (rowsAffected > 0) {
                DebugLogger.d(TAG, "联系人目标编辑成功: contactId=$contactId")
                Result.success(EditResult.Success)
            } else {
                DebugLogger.w(TAG, "联系人目标编辑失败: 未找到记录 contactId=$contactId")
                Result.success(EditResult.NotFound)
            }
        } catch (e: Exception) {
            DebugLogger.e(TAG, "联系人目标编辑失败", e)
            Result.success(EditResult.DatabaseError(e))
        }
    }
}
