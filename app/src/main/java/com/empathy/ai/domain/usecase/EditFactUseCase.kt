...
class EditFactUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        try {
            ...
            if (profile == null) {
                DebugLogger.e(TAG, "联系人不存在: contactId=$contactId")
                return@withContext Result.success(EditResult.NotFound)
            }

            // 3. 查找事实 - 添加详细日志
            DebugLogger.d(TAG, "========== 事实查找调试 ==========")
            DebugLogger.d(TAG, "查找目标factId: $factId")
            DebugLogger.d(TAG, "数据库中facts数量: ${profile.facts.size}")
            profile.facts.forEachIndexed { index, f ->
                DebugLogger.d(TAG, "  [$index] id=${f.id}, key=${f.key}, value=${f.value.take(20)}...")
            }
            
            val fact = profile.facts.find { it.id == factId }
            if (fact == null) {
                DebugLogger.e(TAG, "未找到事实! factId=$factId 不在数据库的facts列表中")
                DebugLogger.e(TAG, "可能原因: 1.新添加的事实未持久化 2.id在序列化时丢失 3.内存状态与数据库不同步")
                return@withContext Result.success(EditResult.NotFound)
            }
            DebugLogger.d(TAG, "找到事实: id=${fact.id}, key=${fact.key}")
            ...
            DebugLogger.d(TAG, "事实编辑成功: factId=$factId")
            Result.success(EditResult.Success)
        } catch (e: Exception) {
            DebugLogger.e(TAG, "事实编辑失败", e)
