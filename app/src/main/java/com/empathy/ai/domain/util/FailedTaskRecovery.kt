...
class FailedTaskRecovery @Inject constructor(
    ...
    suspend fun recover(
        ...
        try {
            val pendingTasks = failedTaskRepository.getPendingTasks().getOrNull() ?: return RecoveryResult(0, 0, 0, 0)

            for (task in pendingTasks) {
                if (task.retryCount >= MemoryConstants.MAX_TASK_RETRIES) {
                    Log.w(TAG, "任务 ${task.id} 已超过最大重试次数，放弃")
                ...
                if (success) {
                    failedTaskRepository.deleteTask(task.id)
                    recoveredCount++
                    Log.d(TAG, "成功恢复任务 ${task.id}")
                } else {
                    failedTaskRepository.incrementRetryCount(task.id)
                    failedCount++
                    Log.w(TAG, "任务 ${task.id} 重试失败，已更新重试次数")
                }
            }
    ...
    suspend fun saveFailedTask(
        ...
        try {
            ...
            Log.d(TAG, "保存失败任务: contactId=$contactId, date=$summaryDate")
        } catch (e: Exception) {
            Log.e(TAG, "保存失败任务异常", e)
        }
    }
}
