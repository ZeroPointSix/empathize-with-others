 ...
 *
 * 测试提示词文件存储管理 (PRD-00005 提示词管理系统):
 * - 首次读取创建默认配置 - 验证v1/v2配置文件自动生成
 * - 写入后读取一致性 - 文件持久化正确性
 * - 缓存机制 - readGlobalConfig使用内存缓存优化性能
 * - 并发读写安全性 - 多协程访问时的线程安全
 * - 备份机制 - writeGlobalConfig前自动创建备份
 * - 错误恢复 - 文件损坏时返回默认配置
 * - 版本迁移 - v1配置中的变量占位符自动清空
 *
 * 业务规则 (TDD-00005/4.1):
 * - 配置文件存储于app内部存储prompts目录
 * - 使用JSON格式存储，支持中文等特殊字符
 * - 版本升级时自动迁移旧配置
 *
 * 设计权衡 (TDD-00005/4.1):
 * - 缓存使用协程内部的synchronized保证线程安全
 * - 备份使用时间戳命名，便于问题排查
 *
 * 任务追踪:
 * - PRD-00005 - 提示词管理系统需求
