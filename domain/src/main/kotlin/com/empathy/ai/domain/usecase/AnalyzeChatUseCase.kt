...
class AnalyzeChatUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            ...
            logger.d(TAG, "历史配置: historyCount=$historyCount, contactId=$contactId")
            ...
            logger.d(TAG, "历史上下文长度: ${historyContext.length}, 内容预览: ${historyContext.take(200)}")
            ...
            logger.d(TAG, "保存用户输入(带身份前缀): contactId=$contactId, logId=$conversationLogId")
            ...
            val userProfileContext = try {
                ...
                logger.w(TAG, "获取用户画像上下文失败，降级为空上下文")
                ""  // 降级：用户画像获取失败不影响主流程
            }
            
            logger.d(TAG, "用户画像上下文长度: ${userProfileContext.length}")

            // 【TD-00016】获取当前对话主题
            val activeTopic = try {
                topicRepository.getActiveTopic(contactId)
            } catch (e: Exception) {
                logger.w(TAG, "获取对话主题失败，降级为无主题")
                null  // 降级：主题获取失败不影响主流程
            }
            
            logger.d(TAG, "当前对话主题: ${activeTopic?.content?.take(50) ?: "无"}")
            ...
            logger.e(TAG, "分析失败，但用户输入已保存: logId=$conversationLogId", e)
            Result.failure(e)
        }
    }
    ...
    private suspend fun buildHistoryContext(contactId: String, limit: Int): String {
        return try {
            ...
            logger.e(TAG, "构建历史上下文失败，降级为空历史", e)
            ""  // 降级：返回空历史，不影响主流程
        }
    }
