...
> 一套适用于任何项目的 Claude Code 技能集合，涵盖代码审查、架构设计、测试、调试、编码规范、重构、Git 操作、文档编写、错误跟踪、数据库操作、API 设计、前端/后端开发、中间件、性能优化、安全、头脑风暴、计划编写和完成前验证、深度代码分析、插件开发、文档处理、多智能体并行探索等 43 个技能。
...
**multi-agent-explorer** 是一套基于 Git Worktree 的多智能体并行开发框架，支持在独立工作树中运行多个 AI 智能体进行探索性开发。

### 包含的智能体

| 智能体 | 用途 | Slash 命令 |
|--------|------|------------|
| bugfix-explorer | Bug 修复探索 | /explore-bugfix |
| feature-explorer | 功能开发探索 | /explore-feature |
| free-explorer | 自由创新探索 | /explore-free |
| architecture-reviewer | 架构审查 | /explore-arch |
| test-explorer | 测试扩展 | /explore-test |
| worktree-manager | 工作树管理 | /explore-manage |

### 核心特点

- **沙盒环境**：所有探索在独立工作树中进行，不影响主分支
- **自主工作**：智能体无需频繁询问用户，按最佳实践自主决策
- **报告驱动**：所有探索结果以报告形式输出，供主 Agent 参考
- **决策日志**：每个智能体必须维护决策日志，记录所有决策过程和学习
- **影响范围评估**：🆕 在开始修改前评估影响范围，支持并行开发规划
- **人类审查**：最终是否合并由人类决定

### 🆕 影响范围评估机制

**影响范围评估是支持多智能体并行开发的核心机制**。在开始任何修改前，必须先评估：

1. 将要修改、新增、删除的文件
2. 与其他任务的冲突
3. 并行性结论

这样可以在 PRD 阶段就规划好哪些任务可以并行执行，哪些必须串行执行。

详见 [impact-analysis-guide.md](./references/impact-analysis-guide.md)

### 🆕 决策日志机制

**决策日志是多智能体系统的核心学习机制**。即使任务失败，详细的决策日志也是宝贵的知识资产。

每个智能体在工作时必须：
1. 创建 `DECISION_JOURNAL.md` 文件
2. 实时记录遇到的问题、考虑的方案、做出的决策
3. 记录失败的尝试和学到的教训
4. 为后续智能体提供建议

详见 [decision-journal-guide.md](./references/decision-journal-guide.md)
...
详见 [multi-agent-explorer/SKILL.md](./multi-agent-explorer/SKILL.md)
