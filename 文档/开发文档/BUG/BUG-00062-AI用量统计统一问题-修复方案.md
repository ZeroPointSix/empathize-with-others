# BUG-00062：AI用量统计统一问题 - 修复方案

## 文档信息

| 属性 | 值 |
|------|-----|
| 文档编号 | BUG-00062-FIX |
| 问题编号 | BUG-00062 |
| 创建日期 | 2026-01-10 |
| 优先级 | P1-高 |
| 预计工时 | 3小时 |
| 状态 | 待实现 |

---

## 1. 问题分析

### 1.1 问题根因

通过代码分析，发现以下两个 AI 调用方法**缺少用量统计**：

| 方法 | 文件位置 | 调用场景 | 统计状态 |
|------|----------|----------|----------|
| `generateText` | `AiRepositoryImpl.kt:572-606` | AI总结 | ❌ 无统计 |
| `generateTextStream` | `AiRepositoryImpl.kt:1230-1275` | AI军师对话 | ❌ 无统计 |

### 1.2 现有统计覆盖情况

| 方法 | 统计状态 | 调用场景 |
|------|----------|----------|
| `analyzeChat` | ✅ 有统计 | 悬浮窗分析 |
| `polishDraft` | ✅ 有统计 | 悬浮窗润色 |
| `generateReply` | ✅ 有统计 | 悬浮窗回复 |
| `queryKnowledge` | ✅ 有统计 | 悬浮窗知识查询 |
| `generateText` | ❌ 无统计 | AI总结（AiSummaryProcessor） |
| `generateTextStream` | ❌ 无统计 | AI军师对话（SendAdvisorMessageStreamingUseCase） |

### 1.3 调用链路分析

#### AI总结调用链路
```
SummarizeDailyConversationsUseCase.invoke()
    ↓
AiSummaryProcessor.process()
    ↓
aiRepository.generateText()  ← 缺少统计
```

#### AI军师调用链路
```
AiAdvisorChatViewModel.sendMessage()
    ↓
SendAdvisorMessageStreamingUseCase.invoke()
    ↓
aiRepository.generateTextStream()  ← 缺少统计
    ↓
Flow<AiStreamChunk> → Complete/Error
```

---

## 2. 修复方案

### 2.1 方案概述

采用**混合方案**：
- **generateText**：在 Repository 层添加统计（参考 polishDraft 实现）
- **generateTextStream**：在 UseCase 层添加统计（因为流式响应的 token 信息在 Complete 中）

### 2.2 修改清单

#### 修改1：AiRepositoryImpl.generateText

**文件**：`data/src/main/kotlin/com/empathy/ai/data/repository/AiRepositoryImpl.kt`

**修改内容**：
```kotlin
override suspend fun generateText(
    provider: AiProvider,
    prompt: String,
    systemInstruction: String
): Result<String> {
    val startTime = System.currentTimeMillis()  // 🆕 添加
    val model = selectModel(provider)           // 🆕 移到外层
    
    return try {
        val url = buildChatCompletionsUrl(provider.baseUrl)
        val headers = mapOf(
            "Authorization" to "Bearer ${provider.apiKey}",
            "Content-Type" to "application/json"
        )
        val messages = listOf(
            MessageDto(role = "system", content = systemInstruction),
            MessageDto(role = "user", content = prompt)
        )
        val request = ChatRequestDto(
            model = model,
            messages = messages,
            temperature = 0.7,
            stream = false
        )

        val response = withRetry { api.chatCompletion(url, headers, request) }
        val content = response.choices.firstOrNull()?.message?.content
            ?: return Result.failure(Exception("Empty response from AI"))
        
        // 🆕 记录成功的用量
        val requestTimeMs = System.currentTimeMillis() - startTime
        recordUsage(
            providerId = provider.id,
            providerName = provider.name,
            modelId = model,
            promptTokens = estimateTokens(systemInstruction + prompt),
            completionTokens = estimateTokens(content),
            requestTimeMs = requestTimeMs,
            isSuccess = true
        )
        
        Result.success(content)

    } catch (e: HttpException) {
        val errorBody = try { e.response()?.errorBody()?.string() ?: "No error body" } catch (ex: Exception) { "Failed to read error body" }
        // 🆕 记录失败的用量
        val requestTimeMs = System.currentTimeMillis() - startTime
        recordUsage(
            providerId = provider.id,
            providerName = provider.name,
            modelId = model,
            promptTokens = estimateTokens(systemInstruction + prompt),
            completionTokens = 0,
            requestTimeMs = requestTimeMs,
            isSuccess = false,
            errorMessage = "HTTP ${e.code()}"
        )
        Result.failure(Exception("HTTP ${e.code()}: $errorBody"))
    } catch (e: Exception) {
        Log.e("AiRepositoryImpl", "文本生成失败", e)
        // 🆕 记录失败的用量
        val requestTimeMs = System.currentTimeMillis() - startTime
        recordUsage(
            providerId = provider.id,
            providerName = provider.name,
            modelId = model,
            promptTokens = estimateTokens(systemInstruction + prompt),
            completionTokens = 0,
            requestTimeMs = requestTimeMs,
            isSuccess = false,
            errorMessage = e.message
        )
        Result.failure(e)
    }
}
```

#### 修改2：SendAdvisorMessageStreamingUseCase

**文件**：`domain/src/main/kotlin/com/empathy/ai/domain/usecase/SendAdvisorMessageStreamingUseCase.kt`

**修改内容**：

1. 添加依赖注入：
```kotlin
class SendAdvisorMessageStreamingUseCase @Inject constructor(
    private val aiAdvisorRepository: AiAdvisorRepository,
    private val aiRepository: AiRepository,
    private val contactRepository: ContactRepository,
    private val aiProviderRepository: AiProviderRepository,
    private val brainTagRepository: BrainTagRepository,
    private val apiUsageRepository: ApiUsageRepository,  // 🆕 新增
    private val logger: Logger
)
```

2. 在 Complete 处理块中添加用量记录：
```kotlin
is AiStreamChunk.Complete -> {
    val finalContent = chunk.fullText.ifEmpty { contentBuilder.toString() }
    
    // 更新Block内容
    aiAdvisorRepository.updateBlockContent(
        blockId = mainTextBlock.id,
        content = finalContent,
        status = MessageBlockStatus.SUCCESS
    )
    
    // 更新Message的content和status
    aiAdvisorRepository.updateMessageContentAndStatus(
        messageId = aiMessageId,
        content = finalContent,
        status = SendStatus.SUCCESS
    )
    
    // 🆕 记录成功的用量
    val requestTimeMs = System.currentTimeMillis() - startTime
    recordUsageForAdvisor(
        provider = provider,
        promptTokens = chunk.usage?.promptTokens ?: estimateTokens(prompt),
        completionTokens = chunk.usage?.completionTokens ?: estimateTokens(finalContent),
        requestTimeMs = requestTimeMs,
        isSuccess = true
    )
    
    emit(StreamingState.Completed(finalContent, chunk.usage))
}
```

3. 在 Error 处理块中添加用量记录：
```kotlin
is AiStreamChunk.Error -> {
    val errorContent = "[AI响应失败: ${chunk.error.message ?: "未知错误"}]"
    aiAdvisorRepository.updateMessageContentAndStatus(
        messageId = aiMessageId,
        content = errorContent,
        status = SendStatus.FAILED
    )
    
    // 🆕 记录失败的用量
    val requestTimeMs = System.currentTimeMillis() - startTime
    recordUsageForAdvisor(
        provider = provider,
        promptTokens = estimateTokens(prompt),
        completionTokens = 0,
        requestTimeMs = requestTimeMs,
        isSuccess = false,
        errorMessage = chunk.error.message
    )
    
    emit(StreamingState.Error(chunk.error))
}
```

4. 添加辅助方法：
```kotlin
/**
 * 记录AI军师的用量
 */
private suspend fun recordUsageForAdvisor(
    provider: AiProvider,
    promptTokens: Int,
    completionTokens: Int,
    requestTimeMs: Long,
    isSuccess: Boolean,
    errorMessage: String? = null
) {
    try {
        apiUsageRepository.recordUsage(
            ApiUsageRecord(
                id = UUID.randomUUID().toString(),
                providerId = provider.id,
                providerName = provider.name,
                modelId = provider.models.firstOrNull() ?: "unknown",
                modelName = provider.models.firstOrNull() ?: "unknown",
                promptTokens = promptTokens,
                completionTokens = completionTokens,
                totalTokens = promptTokens + completionTokens,
                requestTimeMs = requestTimeMs,
                isSuccess = isSuccess,
                errorMessage = errorMessage,
                createdAt = System.currentTimeMillis()
            )
        )
    } catch (e: Exception) {
        logger.w(TAG, "记录AI军师用量失败: ${e.message}")
    }
}

/**
 * 估算Token数量
 */
private fun estimateTokens(text: String): Int {
    val chineseCount = text.count { it.code in 0x4E00..0x9FFF }
    val otherCount = text.length - chineseCount
    return (chineseCount / 1.5 + otherCount / 4.0).toInt().coerceAtLeast(1)
}
```

#### 修改3：更新DI模块

**文件**：`app/src/main/java/com/empathy/ai/di/AiAdvisorModule.kt`

**修改内容**：
```kotlin
@Provides
@Singleton
fun provideSendAdvisorMessageStreamingUseCase(
    aiAdvisorRepository: AiAdvisorRepository,
    aiRepository: AiRepository,
    contactRepository: ContactRepository,
    aiProviderRepository: AiProviderRepository,
    brainTagRepository: BrainTagRepository,
    apiUsageRepository: ApiUsageRepository,  // 🆕 新增
    logger: Logger
): SendAdvisorMessageStreamingUseCase {
    return SendAdvisorMessageStreamingUseCase(
        aiAdvisorRepository,
        aiRepository,
        contactRepository,
        aiProviderRepository,
        brainTagRepository,
        apiUsageRepository,  // 🆕 新增
        logger
    )
}
```

---

## 3. 任务清单

| 任务ID | 描述 | 文件 | 预计工时 | 优先级 | 依赖 |
|--------|------|------|----------|--------|------|
| T1 | 修改 generateText 添加用量统计 | `AiRepositoryImpl.kt` | 0.5h | P0 | - |
| T2 | 修改 SendAdvisorMessageStreamingUseCase 添加依赖注入 | `SendAdvisorMessageStreamingUseCase.kt` | 0.25h | P0 | - |
| T3 | 修改 SendAdvisorMessageStreamingUseCase 添加用量记录 | `SendAdvisorMessageStreamingUseCase.kt` | 0.5h | P0 | T2 |
| T4 | 更新 AiAdvisorModule DI配置 | `AiAdvisorModule.kt` | 0.25h | P0 | T2 |
| T5 | 编写 generateText 用量统计测试 | `AiRepositoryImplTest.kt` | 0.5h | P1 | T1 |
| T6 | 编写 SendAdvisorMessageStreamingUseCase 用量统计测试 | `SendAdvisorMessageStreamingUseCaseTest.kt` | 0.5h | P1 | T3 |
| T7 | 集成测试验证 | - | 0.5h | P1 | T1-T6 |

**总计**：3小时

---

## 4. 测试用例

### 4.1 单元测试

#### T5: AiRepositoryImpl.generateText 用量统计测试

```kotlin
@Test
fun `generateText should record usage on success`() = runTest {
    // Given
    val provider = createTestProvider()
    val prompt = "测试提示词"
    val systemInstruction = "系统指令"
    val expectedResponse = "AI响应内容"
    
    coEvery { api.chatCompletion(any(), any(), any()) } returns createMockResponse(expectedResponse)
    
    // When
    val result = repository.generateText(provider, prompt, systemInstruction)
    
    // Then
    assertTrue(result.isSuccess)
    coVerify { 
        apiUsageRepository.recordUsage(match { 
            it.providerId == provider.id && 
            it.isSuccess == true &&
            it.promptTokens > 0 &&
            it.completionTokens > 0
        })
    }
}

@Test
fun `generateText should record usage on failure`() = runTest {
    // Given
    val provider = createTestProvider()
    coEvery { api.chatCompletion(any(), any(), any()) } throws IOException("网络错误")
    
    // When
    val result = repository.generateText(provider, "prompt", "system")
    
    // Then
    assertTrue(result.isFailure)
    coVerify { 
        apiUsageRepository.recordUsage(match { 
            it.isSuccess == false &&
            it.errorMessage != null
        })
    }
}
```

#### T6: SendAdvisorMessageStreamingUseCase 用量统计测试

```kotlin
@Test
fun `invoke should record usage on streaming complete`() = runTest {
    // Given
    val contactId = "contact-1"
    val sessionId = "session-1"
    val userMessage = "你好"
    val provider = createTestProvider()
    
    coEvery { aiProviderRepository.getDefaultProvider() } returns Result.success(provider)
    coEvery { aiRepository.generateTextStream(any(), any(), any()) } returns flowOf(
        AiStreamChunk.Started,
        AiStreamChunk.TextDelta("你好"),
        AiStreamChunk.Complete("你好，有什么可以帮助你的？", TokenUsage(10, 20))
    )
    
    // When
    useCase(contactId, sessionId, userMessage).collect()
    
    // Then
    coVerify { 
        apiUsageRepository.recordUsage(match { 
            it.providerId == provider.id && 
            it.isSuccess == true &&
            it.promptTokens == 10 &&
            it.completionTokens == 20
        })
    }
}

@Test
fun `invoke should record usage on streaming error`() = runTest {
    // Given
    coEvery { aiRepository.generateTextStream(any(), any(), any()) } returns flowOf(
        AiStreamChunk.Started,
        AiStreamChunk.Error(IOException("网络错误"))
    )
    
    // When
    useCase("contact-1", "session-1", "你好").collect()
    
    // Then
    coVerify { 
        apiUsageRepository.recordUsage(match { 
            it.isSuccess == false &&
            it.errorMessage == "网络错误"
        })
    }
}
```

### 4.2 集成测试

#### T7: 端到端验证

```kotlin
@Test
fun `AI军师对话后用量统计应显示记录`() {
    // 1. 进入AI军师界面
    // 2. 发送一条消息
    // 3. 等待AI响应完成
    // 4. 进入设置 → AI配置 → 用量统计
    // 5. 验证显示了本次对话的用量记录
}

@Test
fun `AI总结执行后用量统计应显示记录`() {
    // 1. 确保有未总结的对话记录
    // 2. 触发每日总结（或手动总结）
    // 3. 等待总结完成
    // 4. 进入设置 → AI配置 → 用量统计
    // 5. 验证显示了本次总结的用量记录
}
```

---

## 5. 风险评估

| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|----------|
| 流式响应 token 信息不准确 | 统计数据偏差 | 中 | 使用 estimateTokens 作为备选 |
| 依赖注入变更导致编译错误 | 构建失败 | 低 | 更新 DI 模块配置 |
| 性能影响 | 轻微延迟 | 低 | 异步记录，不阻塞主流程 |
| 记录失败导致异常 | 功能中断 | 低 | try-catch 包裹，仅记录警告日志 |

---

## 6. 验收标准

### 6.1 功能验收

- [ ] AI军师对话后，用量统计页面显示对应记录
- [ ] AI总结执行后，用量统计页面显示对应记录
- [ ] 失败的请求也被正确记录（isSuccess=false）
- [ ] Token 计数基本准确（误差 < 20%）
- [ ] 用量统计页面的按服务商/按模型分类正确

### 6.2 技术验收

- [ ] 单元测试全部通过
- [ ] 无编译错误
- [ ] 无运行时崩溃
- [ ] 日志输出正常

---

## 7. 相关文档

- [BUG-00062-AI用量统计统一问题.md](./BUG-00062-AI用量统计统一问题.md) - 问题描述
- [TDD-00025-AI配置功能完善技术设计.md](../TDD/TDD-00025-AI配置功能完善技术设计.md) - 用量统计原始设计
- [FD-00025-AI配置功能完善功能设计.md](../FD/FD-00025-AI配置功能完善功能设计.md) - 功能设计文档

---

**文档版本**: 1.0
**最后更新**: 2026-01-10
