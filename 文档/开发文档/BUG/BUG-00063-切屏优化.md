...
3. 安装到 MuMu: `adb -s 192.0.2.1:7555 install -r app-debug.apk` 成功。
4. 启动应用: `adb -s 192.0.2.1:7555 shell am start -n com.empathy.ai/com.empathy.ai.ui.MainActivity` 成功。
5. 重新构建与安装: `./gradlew assembleDebug` 与 `adb -s 192.0.2.1:7555 install -r app-debug.apk` 成功。
