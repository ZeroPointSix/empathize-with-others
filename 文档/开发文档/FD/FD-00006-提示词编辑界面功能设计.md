# FD-00006: 提示词编辑界面功能设计

> **文档类型**: 功能设计文档 (FD)  
> **版本**: 1.1  
> **创建日期**: 2025-12-16  
> **最后更新**: 2025-12-16  
> **负责人**: Kiro  
> **状态**: ✅ 审查通过  
> **优先级**: 🟡 中  
> **关联PRD**: PRD-00006  
> **关联FD**: FD-00005（提示词管理系统功能设计）

---

## 📜 版本历史

| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| 1.0 | 2025-12-16 | Kiro | 初始版本，包含完整UI架构设计 |
| 1.1 | 2025-12-16 | Kiro | 根据DR-00012审查报告补充：国际化设计、性能优化、扩展测试用例、版本兼容性 |

---

## 📋 文档概述

本文档详细描述提示词编辑界面的技术架构、组件设计、状态管理和实现细节。

**设计目标**：
- 打造极简、聚焦的提示词编辑体验
- 沿用产品标志性的情感化设计语言（暖色光晕 + 磨砂玻璃）
- 确保键盘弹起时的良好交互体验
- 支持全局提示词和联系人提示词两种编辑模式

**依赖库版本**：
- Jetpack Compose BOM: 2024.12.01
- Material 3: 1.3.1
- Navigation Compose: 2.8.5
- Hilt: 2.52


---

## 📑 目录

1. [整体架构](#整体架构)
2. [数据模型设计](#数据模型设计)
3. [核心组件设计](#核心组件设计)
4. [状态管理设计](#状态管理设计)
5. [导航设计](#导航设计)
6. [视觉组件设计](#视觉组件设计)
7. [交互设计](#交互设计)
8. [错误处理机制](#错误处理机制)
9. [测试策略](#测试策略)
10. [国际化设计](#国际化设计)
11. [性能优化设计](#性能优化设计)
12. [扩展测试用例](#扩展测试用例)
13. [版本兼容性](#版本兼容性)
14. [实施计划](#实施计划)
8. [错误处理机制](#错误处理机制)
9. [测试策略](#测试策略)
10. [实施计划](#实施计划)

---

## 🏗️ 整体架构

### 架构层次

```
┌─────────────────────────────────────────────────────────┐
│                    Presentation Layer                    │
│  ┌──────────────────────────────────────────────────┐  │
│  │              PromptEditorScreen                   │  │
│  │  ┌────────────────────────────────────────────┐  │  │
│  │  │           EmotionalBackground              │  │  │
│  │  │  ┌──────────────────────────────────────┐  │  │  │
│  │  │  │         GlassmorphicCard             │  │  │  │
│  │  │  │  ┌────────────────────────────────┐  │  │  │  │
│  │  │  │  │      PromptEditorTopBar        │  │  │  │  │
│  │  │  │  ├────────────────────────────────┤  │  │  │  │
│  │  │  │  │      PromptInputField          │  │  │  │  │
│  │  │  │  ├────────────────────────────────┤  │  │  │  │
│  │  │  │  │      CharacterCounter          │  │  │  │  │
│  │  │  │  └────────────────────────────────┘  │  │  │  │
│  │  │  └──────────────────────────────────────┘  │  │  │
│  │  └────────────────────────────────────────────┘  │  │
│  └──────────────────────────────────────────────────┘  │
│                          ↓                              │
│  ┌──────────────────────────────────────────────────┐  │
│  │           PromptEditorViewModel                   │  │
│  │  - UiState 管理                                   │  │
│  │  - 事件处理                                       │  │
│  │  - 数据加载/保存                                  │  │
│  └──────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│                     Domain Layer                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │PromptRepository│ │PromptValidator│ │PromptScene  │  │
│  │   (接口)      │  │  (验证器)    │  │  (场景枚举) │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────┘
```

### 模块划分

```
presentation/
├── ui/screen/prompt/
│   ├── PromptEditorScreen.kt           # 主界面
│   ├── PromptEditorUiState.kt          # UI状态
│   ├── PromptEditorUiEvent.kt          # UI事件
│   └── component/
│       ├── PromptEditorTopBar.kt       # 顶部导航栏
│       ├── PromptInputField.kt         # 输入框组件
│       ├── CharacterCounter.kt         # 字符计数器
│       └── DiscardConfirmDialog.kt     # 放弃确认对话框
├── viewmodel/
│   └── PromptEditorViewModel.kt        # ViewModel
├── navigation/
│   └── PromptEditorNavigation.kt       # 导航定义
└── theme/
    └── BrandColors.kt                  # 品牌色定义（扩展）
```


---

## 📊 数据模型设计

### 1. PromptEditorUiState（UI状态）

**职责**：管理编辑器界面的完整状态

```kotlin
data class PromptEditorUiState(
    // 编辑模式
    val editMode: PromptEditMode = PromptEditMode.GlobalScene(PromptScene.ANALYZE),
    
    // 内容状态
    val originalPrompt: String = "",          // 原始提示词（用于检测修改）
    val currentPrompt: String = "",           // 当前编辑的提示词
    val placeholderText: String = "",         // 动态占位符文案
    
    // 加载状态
    val isLoading: Boolean = false,           // 初始加载状态
    val isSaving: Boolean = false,            // 保存状态
    
    // 对话框状态
    val showDiscardDialog: Boolean = false,   // 显示放弃修改对话框
    
    // 错误状态
    val errorMessage: String? = null          // 错误信息
) {
    // 计算属性
    val charCount: Int get() = currentPrompt.length
    val isOverLimit: Boolean get() = charCount > MAX_PROMPT_LENGTH
    val isNearLimit: Boolean get() = charCount > WARN_PROMPT_LENGTH
    val hasUnsavedChanges: Boolean get() = currentPrompt != originalPrompt
    val canSave: Boolean get() = !isOverLimit && !isSaving && !isLoading
    
    // 标题文案
    val titleText: String get() = when (editMode) {
        is PromptEditMode.GlobalScene -> "编辑指令"
        is PromptEditMode.ContactCustom -> "编辑专属指令"
    }
    
    companion object {
        const val MAX_PROMPT_LENGTH = 1000
        const val WARN_PROMPT_LENGTH = 800
    }
}
```

### 2. PromptEditMode（编辑模式）

**职责**：区分全局场景编辑和联系人专属编辑

```kotlin
sealed class PromptEditMode {
    /**
     * 全局场景提示词编辑
     * @param scene 场景类型
     */
    data class GlobalScene(val scene: PromptScene) : PromptEditMode()
    
    /**
     * 联系人专属提示词编辑
     * @param contactId 联系人ID
     * @param contactName 联系人名称（用于显示）
     */
    data class ContactCustom(
        val contactId: String,
        val contactName: String
    ) : PromptEditMode()
}
```

### 3. PromptEditorUiEvent（UI事件）

**职责**：定义用户交互产生的所有事件

```kotlin
sealed class PromptEditorUiEvent {
    // 内容编辑
    data class UpdatePrompt(val text: String) : PromptEditorUiEvent()
    
    // 操作按钮
    object SavePrompt : PromptEditorUiEvent()
    object CancelEdit : PromptEditorUiEvent()
    
    // 对话框交互
    object ConfirmDiscard : PromptEditorUiEvent()
    object DismissDiscardDialog : PromptEditorUiEvent()
    
    // 错误处理
    object ClearError : PromptEditorUiEvent()
}
```

### 4. PromptEditorResult（编辑结果）

**职责**：向调用方返回编辑结果

```kotlin
sealed class PromptEditorResult {
    object Saved : PromptEditorResult()      // 保存成功
    object Cancelled : PromptEditorResult()  // 用户取消
}
```


---

## 🔧 核心组件设计

### 1. PromptEditorViewModel

**职责**：管理编辑器的业务逻辑和状态

```kotlin
@HiltViewModel
class PromptEditorViewModel @Inject constructor(
    private val promptRepository: PromptRepository,
    private val promptValidator: PromptValidator,
    savedStateHandle: SavedStateHandle
) : ViewModel() {

    // 从导航参数获取编辑模式
    private val editMode: PromptEditMode = parseEditMode(savedStateHandle)
    
    private val _uiState = MutableStateFlow(
        PromptEditorUiState(
            editMode = editMode,
            placeholderText = getPlaceholderText(editMode)
        )
    )
    val uiState: StateFlow<PromptEditorUiState> = _uiState.asStateFlow()
    
    // 编辑结果回调
    private val _result = MutableSharedFlow<PromptEditorResult>()
    val result: SharedFlow<PromptEditorResult> = _result.asSharedFlow()
    
    init {
        loadPrompt()
    }
    
    fun onEvent(event: PromptEditorUiEvent) {
        when (event) {
            is PromptEditorUiEvent.UpdatePrompt -> updatePrompt(event.text)
            is PromptEditorUiEvent.SavePrompt -> savePrompt()
            is PromptEditorUiEvent.CancelEdit -> handleCancel()
            is PromptEditorUiEvent.ConfirmDiscard -> confirmDiscard()
            is PromptEditorUiEvent.DismissDiscardDialog -> dismissDiscardDialog()
            is PromptEditorUiEvent.ClearError -> clearError()
        }
    }
    
    private fun loadPrompt() {
        viewModelScope.launch {
            _uiState.update { it.copy(isLoading = true) }
            
            try {
                val prompt = when (val mode = editMode) {
                    is PromptEditMode.GlobalScene -> 
                        promptRepository.getGlobalPrompt(mode.scene)
                    is PromptEditMode.ContactCustom -> 
                        promptRepository.getContactPrompt(mode.contactId) ?: ""
                }
                
                _uiState.update { 
                    it.copy(
                        originalPrompt = prompt,
                        currentPrompt = prompt,
                        isLoading = false
                    )
                }
            } catch (e: Exception) {
                _uiState.update { 
                    it.copy(
                        isLoading = false,
                        errorMessage = "加载失败: ${e.message}"
                    )
                }
            }
        }
    }
    
    private fun updatePrompt(text: String) {
        _uiState.update { it.copy(currentPrompt = text) }
    }
    
    private fun savePrompt() {
        val currentState = _uiState.value
        if (!currentState.canSave) return
        
        viewModelScope.launch {
            _uiState.update { it.copy(isSaving = true) }
            
            try {
                val result = when (val mode = editMode) {
                    is PromptEditMode.GlobalScene -> 
                        promptRepository.saveGlobalPrompt(mode.scene, currentState.currentPrompt)
                    is PromptEditMode.ContactCustom -> 
                        promptRepository.saveContactPrompt(
                            mode.contactId, 
                            currentState.currentPrompt.ifBlank { null }
                        )
                }
                
                result.fold(
                    onSuccess = {
                        _result.emit(PromptEditorResult.Saved)
                    },
                    onFailure = { e ->
                        _uiState.update { 
                            it.copy(
                                isSaving = false,
                                errorMessage = "保存失败: ${e.message}"
                            )
                        }
                    }
                )
            } catch (e: Exception) {
                _uiState.update { 
                    it.copy(
                        isSaving = false,
                        errorMessage = "保存失败: ${e.message}"
                    )
                }
            }
        }
    }
    
    private fun handleCancel() {
        if (_uiState.value.hasUnsavedChanges) {
            _uiState.update { it.copy(showDiscardDialog = true) }
        } else {
            viewModelScope.launch {
                _result.emit(PromptEditorResult.Cancelled)
            }
        }
    }
    
    private fun confirmDiscard() {
        viewModelScope.launch {
            _uiState.update { it.copy(showDiscardDialog = false) }
            _result.emit(PromptEditorResult.Cancelled)
        }
    }
    
    private fun dismissDiscardDialog() {
        _uiState.update { it.copy(showDiscardDialog = false) }
    }
    
    private fun clearError() {
        _uiState.update { it.copy(errorMessage = null) }
    }
    
    companion object {
        /**
         * 获取动态占位符文案
         */
        fun getPlaceholderText(editMode: PromptEditMode): String {
            return when (editMode) {
                is PromptEditMode.ContactCustom -> 
                    "例如：和她聊天时要注意避开前任话题..."
                is PromptEditMode.GlobalScene -> when (editMode.scene) {
                    PromptScene.ANALYZE -> "例如：请帮我分析她这句话的潜台词..."
                    PromptScene.CHECK -> "例如：检查时要特别注意工作相关话题..."
                    PromptScene.EXTRACT -> "例如：重点关注对方提到的兴趣爱好..."
                    PromptScene.SUMMARY -> "例如：总结今天的对话亮点和需要注意的地方..."
                }
            }
        }
    }
}
```


---

## 🎨 视觉组件设计

### 1. PromptEditorScreen（主界面）

**职责**：组合所有子组件，处理键盘避让

```kotlin
@Composable
fun PromptEditorScreen(
    viewModel: PromptEditorViewModel = hiltViewModel(),
    onNavigateBack: () -> Unit
) {
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()
    
    // 监听编辑结果
    LaunchedEffect(Unit) {
        viewModel.result.collect { result ->
            when (result) {
                is PromptEditorResult.Saved -> {
                    // 显示保存成功 Toast
                    onNavigateBack()
                }
                is PromptEditorResult.Cancelled -> {
                    onNavigateBack()
                }
            }
        }
    }
    
    // 处理返回键
    BackHandler(enabled = uiState.hasUnsavedChanges) {
        viewModel.onEvent(PromptEditorUiEvent.CancelEdit)
    }
    
    Box(
        modifier = Modifier
            .fillMaxSize()
            .imePadding()  // 键盘避让
            .navigationBarsPadding()
    ) {
        // 背景层
        EmotionalBackground(
            emotionType = EmotionType.WARM,
            modifier = Modifier.fillMaxSize()
        )
        
        // 内容层
        PromptEditorContent(
            uiState = uiState,
            onEvent = viewModel::onEvent,
            modifier = Modifier
                .fillMaxSize()
                .padding(16.dp)
        )
        
        // 放弃确认对话框
        if (uiState.showDiscardDialog) {
            DiscardConfirmDialog(
                onConfirm = { viewModel.onEvent(PromptEditorUiEvent.ConfirmDiscard) },
                onDismiss = { viewModel.onEvent(PromptEditorUiEvent.DismissDiscardDialog) }
            )
        }
        
        // 错误提示
        uiState.errorMessage?.let { message ->
            LaunchedEffect(message) {
                // 显示 Snackbar 或 Toast
                delay(3000)
                viewModel.onEvent(PromptEditorUiEvent.ClearError)
            }
        }
    }
}

@Composable
private fun PromptEditorContent(
    uiState: PromptEditorUiState,
    onEvent: (PromptEditorUiEvent) -> Unit,
    modifier: Modifier = Modifier
) {
    GlassmorphicCard(
        modifier = modifier
    ) {
        Column(
            modifier = Modifier
                .fillMaxWidth()
                .padding(horizontal = 20.dp, vertical = 16.dp)
        ) {
            // 顶部导航栏
            PromptEditorTopBar(
                title = uiState.titleText,
                canSave = uiState.canSave,
                isSaving = uiState.isSaving,
                onCancel = { onEvent(PromptEditorUiEvent.CancelEdit) },
                onSave = { onEvent(PromptEditorUiEvent.SavePrompt) }
            )
            
            Spacer(modifier = Modifier.height(16.dp))
            
            // 输入区域
            PromptInputField(
                value = uiState.currentPrompt,
                onValueChange = { onEvent(PromptEditorUiEvent.UpdatePrompt(it)) },
                placeholder = uiState.placeholderText,
                isLoading = uiState.isLoading,
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(1f, fill = false)
            )
            
            Spacer(modifier = Modifier.height(8.dp))
            
            // 字符计数器
            CharacterCounter(
                currentCount = uiState.charCount,
                maxCount = PromptEditorUiState.MAX_PROMPT_LENGTH,
                isNearLimit = uiState.isNearLimit,
                isOverLimit = uiState.isOverLimit,
                modifier = Modifier.align(Alignment.End)
            )
        }
    }
}
```

### 2. PromptEditorTopBar（顶部导航栏）

**职责**：显示取消、标题、完成按钮

```kotlin
@Composable
fun PromptEditorTopBar(
    title: String,
    canSave: Boolean,
    isSaving: Boolean,
    onCancel: () -> Unit,
    onSave: () -> Unit,
    modifier: Modifier = Modifier
) {
    Row(
        modifier = modifier
            .fillMaxWidth()
            .height(44.dp),
        horizontalArrangement = Arrangement.SpaceBetween,
        verticalAlignment = Alignment.CenterVertically
    ) {
        // 取消按钮
        TextButton(
            onClick = onCancel,
            enabled = !isSaving
        ) {
            Text(
                text = "取消",
                style = MaterialTheme.typography.bodyLarge,
                color = MaterialTheme.colorScheme.onSurface.copy(
                    alpha = if (isSaving) 0.4f else 0.8f
                )
            )
        }
        
        // 标题
        Text(
            text = title,
            style = MaterialTheme.typography.titleMedium,
            fontWeight = FontWeight.SemiBold,
            color = MaterialTheme.colorScheme.onSurface
        )
        
        // 完成按钮 / 加载圈
        Box(
            modifier = Modifier.width(64.dp),
            contentAlignment = Alignment.CenterEnd
        ) {
            if (isSaving) {
                CircularProgressIndicator(
                    modifier = Modifier.size(20.dp),
                    color = BrandWarmGold,
                    strokeWidth = 2.dp
                )
            } else {
                TextButton(
                    onClick = onSave,
                    enabled = canSave
                ) {
                    Text(
                        text = "完成",
                        style = MaterialTheme.typography.bodyLarge,
                        fontWeight = FontWeight.SemiBold,
                        color = if (canSave) {
                            BrandWarmGold
                        } else {
                            BrandWarmGold.copy(alpha = 0.4f)
                        }
                    )
                }
            }
        }
    }
}
```


### 3. PromptInputField（输入框组件）

**职责**：提供品牌色光标的多行文本输入，确保光标始终可见

#### 3.1 基础实现（BringIntoViewRequester 方案）

```kotlin
@Composable
fun PromptInputField(
    value: String,
    onValueChange: (String) -> Unit,
    placeholder: String,
    isLoading: Boolean,
    modifier: Modifier = Modifier
) {
    val bringIntoViewRequester = remember { BringIntoViewRequester() }
    val coroutineScope = rememberCoroutineScope()
    val scrollState = rememberScrollState()
    
    // 配置品牌色选中效果
    CompositionLocalProvider(
        LocalTextSelectionColors provides TextSelectionColors(
            handleColor = BrandWarmGold,
            backgroundColor = BrandWarmGold.copy(alpha = 0.3f)
        )
    ) {
        Box(
            modifier = modifier
                .heightIn(min = 200.dp)
                .verticalScroll(scrollState)
        ) {
            if (isLoading) {
                // 加载状态
                Box(
                    modifier = Modifier.fillMaxSize(),
                    contentAlignment = Alignment.Center
                ) {
                    CircularProgressIndicator(
                        color = BrandWarmGold,
                        modifier = Modifier.size(32.dp)
                    )
                }
            } else {
                BasicTextField(
                    value = value,
                    onValueChange = { newText ->
                        onValueChange(newText)
                        // 确保光标位置可见
                        coroutineScope.launch {
                            bringIntoViewRequester.bringIntoView()
                        }
                    },
                    modifier = Modifier
                        .fillMaxWidth()
                        .bringIntoViewRequester(bringIntoViewRequester),
                    textStyle = MaterialTheme.typography.bodyLarge.copy(
                        lineHeight = 24.sp,
                        color = MaterialTheme.colorScheme.onSurface
                    ),
                    cursorBrush = SolidColor(BrandWarmGold),
                    decorationBox = { innerTextField ->
                        Box {
                            if (value.isEmpty()) {
                                Text(
                                    text = placeholder,
                                    style = MaterialTheme.typography.bodyLarge.copy(
                                        lineHeight = 24.sp
                                    ),
                                    color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.4f)
                                )
                            }
                            innerTextField()
                        }
                    }
                )
            }
        }
    }
}
```

#### 3.2 增强实现（onTextLayout 精准定位方案）

> ⚠️ **备选方案**：如果基础实现在长文本场景下光标跟随不够精准，可采用此增强方案

```kotlin
@Composable
fun PromptInputFieldEnhanced(
    value: String,
    onValueChange: (String) -> Unit,
    placeholder: String,
    isLoading: Boolean,
    modifier: Modifier = Modifier
) {
    val textFieldValue = remember(value) {
        TextFieldValue(text = value, selection = TextRange(value.length))
    }
    var currentTextFieldValue by remember { mutableStateOf(textFieldValue) }
    
    val scrollState = rememberScrollState()
    val coroutineScope = rememberCoroutineScope()
    val density = LocalDensity.current
    
    // 记录光标位置用于精准滚动
    var cursorRect by remember { mutableStateOf<Rect?>(null) }
    
    // 配置品牌色选中效果
    CompositionLocalProvider(
        LocalTextSelectionColors provides TextSelectionColors(
            handleColor = BrandWarmGold,
            backgroundColor = BrandWarmGold.copy(alpha = 0.3f)
        )
    ) {
        Box(
            modifier = modifier
                .heightIn(min = 200.dp)
                .verticalScroll(scrollState)
        ) {
            if (isLoading) {
                Box(
                    modifier = Modifier.fillMaxSize(),
                    contentAlignment = Alignment.Center
                ) {
                    CircularProgressIndicator(
                        color = BrandWarmGold,
                        modifier = Modifier.size(32.dp)
                    )
                }
            } else {
                BasicTextField(
                    value = currentTextFieldValue,
                    onValueChange = { newValue ->
                        currentTextFieldValue = newValue
                        onValueChange(newValue.text)
                    },
                    modifier = Modifier.fillMaxWidth(),
                    textStyle = MaterialTheme.typography.bodyLarge.copy(
                        lineHeight = 24.sp,
                        color = MaterialTheme.colorScheme.onSurface
                    ),
                    cursorBrush = SolidColor(BrandWarmGold),
                    // 🆕 通过 onTextLayout 获取精准光标位置
                    onTextLayout = { textLayoutResult ->
                        val cursorOffset = currentTextFieldValue.selection.start
                        if (cursorOffset >= 0 && cursorOffset <= currentTextFieldValue.text.length) {
                            try {
                                cursorRect = textLayoutResult.getCursorRect(cursorOffset)
                                
                                // 计算光标在滚动容器中的位置
                                cursorRect?.let { rect ->
                                    val cursorBottom = rect.bottom
                                    val visibleHeight = scrollState.viewportSize
                                    val currentScroll = scrollState.value
                                    
                                    // 如果光标在可视区域下方，滚动到光标位置
                                    if (cursorBottom > currentScroll + visibleHeight - 50) {
                                        coroutineScope.launch {
                                            scrollState.animateScrollTo(
                                                (cursorBottom - visibleHeight + 100).toInt()
                                                    .coerceAtLeast(0)
                                            )
                                        }
                                    }
                                    // 如果光标在可视区域上方，滚动到光标位置
                                    else if (rect.top < currentScroll) {
                                        coroutineScope.launch {
                                            scrollState.animateScrollTo(
                                                (rect.top - 50).toInt().coerceAtLeast(0)
                                            )
                                        }
                                    }
                                }
                            } catch (e: Exception) {
                                // 忽略越界异常
                            }
                        }
                    },
                    decorationBox = { innerTextField ->
                        Box {
                            if (currentTextFieldValue.text.isEmpty()) {
                                Text(
                                    text = placeholder,
                                    style = MaterialTheme.typography.bodyLarge.copy(
                                        lineHeight = 24.sp
                                    ),
                                    color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.4f)
                                )
                            }
                            innerTextField()
                        }
                    }
                )
            }
        }
    }
}
```

#### 3.3 光标跟随测试场景

> ⚠️ **重要**：在长文本（超过一屏幕）场景下需重点测试以下情况

| 测试场景 | 预期行为 | 验证方法 |
|---------|---------|---------|
| 输入到底部时键盘弹起 | 光标不被键盘遮挡 | 输入超过10行文字，观察最后一行 |
| 快速连续输入 | 滚动平滑，无跳动 | 快速输入50个字符 |
| 粘贴大段文字 | 光标定位到粘贴末尾 | 粘贴500字文本 |
| 点击文本中间位置 | 光标正确定位 | 点击第5行中间 |
| 键盘收起后再弹起 | 光标位置保持 | 收起键盘后再点击输入框 |
| 横竖屏切换 | 光标位置保持可见 | 输入后旋转屏幕 |

#### 3.4 实现选择建议

| 方案 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| 基础方案 (BringIntoViewRequester) | 实现简单，代码量少 | 长文本场景可能不够精准 | MVP 阶段首选 |
| 增强方案 (onTextLayout) | 精准定位，支持复杂场景 | 代码复杂，需要更多测试 | 如基础方案有问题时升级 |

**建议**：先使用基础方案实现，在测试阶段如果发现长文本场景有问题，再升级到增强方案。

### 4. CharacterCounter（字符计数器）

**职责**：显示字符计数和状态颜色

```kotlin
@Composable
fun CharacterCounter(
    currentCount: Int,
    maxCount: Int,
    isNearLimit: Boolean,
    isOverLimit: Boolean,
    modifier: Modifier = Modifier
) {
    val color = when {
        isOverLimit -> Color.Red
        isNearLimit -> BrandWarmOrange
        else -> MaterialTheme.colorScheme.onSurface.copy(alpha = 0.5f)
    }
    
    Text(
        text = "字符计数: $currentCount/$maxCount",
        style = MaterialTheme.typography.bodySmall,
        color = color,
        modifier = modifier
    )
}
```

### 5. DiscardConfirmDialog（放弃确认对话框）

**职责**：确认用户是否放弃未保存的修改

```kotlin
@Composable
fun DiscardConfirmDialog(
    onConfirm: () -> Unit,
    onDismiss: () -> Unit
) {
    AlertDialog(
        onDismissRequest = onDismiss,
        title = {
            Text(
                text = "放弃修改？",
                style = MaterialTheme.typography.titleMedium,
                fontWeight = FontWeight.SemiBold
            )
        },
        text = {
            Text(
                text = "你有未保存的修改，确定要放弃吗？",
                style = MaterialTheme.typography.bodyMedium
            )
        },
        confirmButton = {
            TextButton(onClick = onConfirm) {
                Text(
                    text = "放弃",
                    color = Color.Red
                )
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) {
                Text(
                    text = "继续编辑",
                    color = BrandWarmGold
                )
            }
        }
    )
}
```

### 6. 品牌色定义

**文件位置**：`presentation/theme/BrandColors.kt`

```kotlin
// 品牌暖色系
val BrandWarmGold = Color(0xFFF5A623)      // 暖金色 - 主要操作按钮、光标
val BrandWarmOrange = Color(0xFFFF9500)    // 暖橙色 - 警告状态
val BrandWarmAmber = Color(0xFFFFCC00)     // 琥珀色 - 辅助高亮
```


---

## 🧭 导航设计

### 1. 导航路由定义

**文件位置**：`presentation/navigation/NavRoutes.kt`（扩展）

```kotlin
object NavRoutes {
    // ... 现有路由 ...
    
    // 提示词编辑器路由
    const val PROMPT_EDITOR = "prompt_editor"
    const val PROMPT_EDITOR_ARG_MODE = "mode"
    const val PROMPT_EDITOR_ARG_SCENE = "scene"
    const val PROMPT_EDITOR_ARG_CONTACT_ID = "contactId"
    const val PROMPT_EDITOR_ARG_CONTACT_NAME = "contactName"
    
    // 全局场景编辑路由
    fun promptEditorGlobal(scene: PromptScene): String {
        return "$PROMPT_EDITOR?$PROMPT_EDITOR_ARG_MODE=global&$PROMPT_EDITOR_ARG_SCENE=${scene.name}"
    }
    
    // 联系人专属编辑路由
    fun promptEditorContact(contactId: String, contactName: String): String {
        val encodedName = URLEncoder.encode(contactName, "UTF-8")
        return "$PROMPT_EDITOR?$PROMPT_EDITOR_ARG_MODE=contact&$PROMPT_EDITOR_ARG_CONTACT_ID=$contactId&$PROMPT_EDITOR_ARG_CONTACT_NAME=$encodedName"
    }
}
```

### 2. 导航图配置

**文件位置**：`presentation/navigation/NavGraph.kt`（扩展）

```kotlin
fun NavGraphBuilder.promptEditorNavigation(
    navController: NavController
) {
    composable(
        route = "${NavRoutes.PROMPT_EDITOR}?${NavRoutes.PROMPT_EDITOR_ARG_MODE}={mode}&${NavRoutes.PROMPT_EDITOR_ARG_SCENE}={scene}&${NavRoutes.PROMPT_EDITOR_ARG_CONTACT_ID}={contactId}&${NavRoutes.PROMPT_EDITOR_ARG_CONTACT_NAME}={contactName}",
        arguments = listOf(
            navArgument(NavRoutes.PROMPT_EDITOR_ARG_MODE) {
                type = NavType.StringType
                defaultValue = "global"
            },
            navArgument(NavRoutes.PROMPT_EDITOR_ARG_SCENE) {
                type = NavType.StringType
                nullable = true
                defaultValue = null
            },
            navArgument(NavRoutes.PROMPT_EDITOR_ARG_CONTACT_ID) {
                type = NavType.StringType
                nullable = true
                defaultValue = null
            },
            navArgument(NavRoutes.PROMPT_EDITOR_ARG_CONTACT_NAME) {
                type = NavType.StringType
                nullable = true
                defaultValue = null
            }
        )
    ) {
        PromptEditorScreen(
            onNavigateBack = { navController.popBackStack() }
        )
    }
}
```

### 3. 导航参数解析

**在 ViewModel 中解析**：

```kotlin
private fun parseEditMode(savedStateHandle: SavedStateHandle): PromptEditMode {
    val mode = savedStateHandle.get<String>(NavRoutes.PROMPT_EDITOR_ARG_MODE) ?: "global"
    
    return when (mode) {
        "global" -> {
            val sceneName = savedStateHandle.get<String>(NavRoutes.PROMPT_EDITOR_ARG_SCENE)
                ?: PromptScene.ANALYZE.name
            val scene = PromptScene.valueOf(sceneName)
            PromptEditMode.GlobalScene(scene)
        }
        "contact" -> {
            val contactId = savedStateHandle.get<String>(NavRoutes.PROMPT_EDITOR_ARG_CONTACT_ID)
                ?: throw IllegalArgumentException("contactId is required for contact mode")
            val contactName = savedStateHandle.get<String>(NavRoutes.PROMPT_EDITOR_ARG_CONTACT_NAME)
                ?.let { URLDecoder.decode(it, "UTF-8") }
                ?: "联系人"
            PromptEditMode.ContactCustom(contactId, contactName)
        }
        else -> PromptEditMode.GlobalScene(PromptScene.ANALYZE)
    }
}
```

---

## 🔄 交互流程设计

### 1. 进入编辑器流程

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  设置页面        │     │  联系人详情页    │     │  其他入口       │
│  点击场景卡片    │     │  点击提示词设置  │     │                 │
└────────┬────────┘     └────────┬────────┘     └────────┬────────┘
         │                       │                       │
         ▼                       ▼                       ▼
┌─────────────────────────────────────────────────────────────────┐
│                    NavController.navigate()                      │
│  - promptEditorGlobal(scene)                                    │
│  - promptEditorContact(contactId, contactName)                  │
└─────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    PromptEditorScreen                            │
│  1. ViewModel 解析导航参数                                       │
│  2. 根据 editMode 加载对应提示词                                 │
│  3. 设置动态占位符                                               │
│  4. 显示编辑界面                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2. 保存流程

```
┌─────────────────┐
│  用户点击"完成" │
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────────┐
│  检查 canSave                            │
│  - !isOverLimit                          │
│  - !isSaving                             │
│  - !isLoading                            │
└────────┬────────────────────────────────┘
         │ canSave = true
         ▼
┌─────────────────────────────────────────┐
│  设置 isSaving = true                    │
│  显示加载圈                              │
└────────┬────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────┐
│  调用 PromptRepository                   │
│  - saveGlobalPrompt() 或                 │
│  - saveContactPrompt()                   │
└────────┬────────────────────────────────┘
         │
    ┌────┴────┐
    │         │
    ▼         ▼
┌───────┐ ┌───────────────────────────────┐
│ 成功  │ │ 失败                          │
└───┬───┘ └───────────────┬───────────────┘
    │                     │
    ▼                     ▼
┌───────────────┐ ┌───────────────────────┐
│ emit(Saved)   │ │ 显示错误信息           │
│ 返回上一页    │ │ isSaving = false       │
└───────────────┘ └───────────────────────┘
```

### 3. 取消流程

```
┌─────────────────────────────────────────┐
│  用户点击"取消"或返回键                  │
└────────┬────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────┐
│  检查 hasUnsavedChanges                  │
│  currentPrompt != originalPrompt         │
└────────┬────────────────────────────────┘
         │
    ┌────┴────┐
    │         │
    ▼         ▼
┌───────────┐ ┌───────────────────────────┐
│ 无修改    │ │ 有修改                    │
└─────┬─────┘ └─────────────┬─────────────┘
      │                     │
      ▼                     ▼
┌───────────────┐ ┌───────────────────────┐
│ emit(Cancelled)│ │ 显示放弃确认对话框    │
│ 返回上一页    │ │ showDiscardDialog=true │
└───────────────┘ └───────────────────────┘
                            │
                  ┌─────────┴─────────┐
                  │                   │
                  ▼                   ▼
          ┌───────────────┐ ┌───────────────┐
          │ 点击"放弃"    │ │ 点击"继续编辑"│
          └───────┬───────┘ └───────┬───────┘
                  │                 │
                  ▼                 ▼
          ┌───────────────┐ ┌───────────────┐
          │ emit(Cancelled)│ │ 关闭对话框    │
          │ 返回上一页    │ │ 继续编辑      │
          └───────────────┘ └───────────────┘
```


---

## ⚠️ 错误处理机制

### 1. 错误类型定义

```kotlin
sealed class PromptEditorError {
    object LoadFailed : PromptEditorError()
    object SaveFailed : PromptEditorError()
    object NetworkError : PromptEditorError()
    data class ValidationError(val message: String) : PromptEditorError()
    
    fun toDisplayMessage(): String = when (this) {
        is LoadFailed -> "加载提示词失败，请重试"
        is SaveFailed -> "保存失败，请重试"
        is NetworkError -> "网络错误，请检查网络连接"
        is ValidationError -> message
    }
}
```

### 2. 错误处理策略

| 错误场景 | 处理方式 | 用户反馈 |
|---------|---------|---------|
| 加载失败 | 显示空内容，允许编辑 | Toast "加载失败" |
| 保存失败 | 保持编辑状态，不关闭 | Toast "保存失败，请重试" |
| 字数超限 | 禁用保存按钮 | 字符计数变红 |
| 网络错误 | 重试机制 | Toast "网络错误" |

---

## 🧪 测试策略

### 1. 单元测试

**ViewModel 测试**：

```kotlin
@Test
fun `加载全局提示词成功`() = runTest {
    // Given
    val scene = PromptScene.ANALYZE
    val expectedPrompt = "测试提示词"
    coEvery { promptRepository.getGlobalPrompt(scene) } returns expectedPrompt
    
    // When
    val viewModel = createViewModel(PromptEditMode.GlobalScene(scene))
    
    // Then
    val state = viewModel.uiState.value
    assertEquals(expectedPrompt, state.currentPrompt)
    assertEquals(expectedPrompt, state.originalPrompt)
    assertFalse(state.isLoading)
}

@Test
fun `保存时显示加载状态`() = runTest {
    // Given
    val viewModel = createViewModel()
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt("新内容"))
    
    // When
    viewModel.onEvent(PromptEditorUiEvent.SavePrompt)
    
    // Then
    assertTrue(viewModel.uiState.value.isSaving)
}

@Test
fun `有未保存修改时取消显示确认对话框`() = runTest {
    // Given
    val viewModel = createViewModel()
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt("修改后的内容"))
    
    // When
    viewModel.onEvent(PromptEditorUiEvent.CancelEdit)
    
    // Then
    assertTrue(viewModel.uiState.value.showDiscardDialog)
}

@Test
fun `字数超限时canSave为false`() = runTest {
    // Given
    val viewModel = createViewModel()
    val longText = "a".repeat(1001)
    
    // When
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(longText))
    
    // Then
    assertTrue(viewModel.uiState.value.isOverLimit)
    assertFalse(viewModel.uiState.value.canSave)
}

@Test
fun `动态占位符根据场景正确显示`() = runTest {
    // Given & When
    val analyzeMode = PromptEditMode.GlobalScene(PromptScene.ANALYZE)
    val checkMode = PromptEditMode.GlobalScene(PromptScene.CHECK)
    
    // Then
    val analyzePlaceholder = PromptEditorViewModel.getPlaceholderText(analyzeMode)
    val checkPlaceholder = PromptEditorViewModel.getPlaceholderText(checkMode)
    
    assertTrue(analyzePlaceholder.contains("分析"))
    assertTrue(checkPlaceholder.contains("检查"))
}
```

### 2. UI 测试

```kotlin
@Test
fun `输入文字时字符计数实时更新`() {
    composeTestRule.setContent {
        PromptEditorScreen(...)
    }
    
    // 输入文字
    composeTestRule.onNodeWithTag("prompt_input")
        .performTextInput("测试内容")
    
    // 验证计数更新
    composeTestRule.onNodeWithText("字符计数: 4/1000")
        .assertIsDisplayed()
}

@Test
fun `保存中状态显示加载圈`() {
    composeTestRule.setContent {
        PromptEditorTopBar(
            title = "编辑指令",
            canSave = false,
            isSaving = true,
            onCancel = {},
            onSave = {}
        )
    }
    
    // 验证加载圈显示
    composeTestRule.onNodeWithTag("saving_indicator")
        .assertIsDisplayed()
    
    // 验证"完成"文字不显示
    composeTestRule.onNodeWithText("完成")
        .assertDoesNotExist()
}

@Test
fun `键盘弹起时字符计数不被遮挡`() {
    // 此测试需要在真机或模拟器上运行
    // 验证 imePadding() 正确工作
}
```

---

## 🌐 国际化设计

### 1. 字符串资源文件

**文件位置**：`res/values/strings.xml`（中文）

```xml
<!-- 提示词编辑器 -->
<string name="prompt_editor_title_global">编辑指令</string>
<string name="prompt_editor_title_contact">编辑专属指令</string>
<string name="prompt_editor_cancel">取消</string>
<string name="prompt_editor_save">完成</string>
<string name="prompt_editor_char_count">字符计数: %1$d/%2$d</string>

<!-- 动态占位符 -->
<string name="prompt_placeholder_analyze">例如：请帮我分析她这句话的潜台词...</string>
<string name="prompt_placeholder_check">例如：检查时要特别注意工作相关话题...</string>
<string name="prompt_placeholder_extract">例如：重点关注对方提到的兴趣爱好...</string>
<string name="prompt_placeholder_summary">例如：总结今天的对话亮点和需要注意的地方...</string>
<string name="prompt_placeholder_contact">例如：和她聊天时要注意避开前任话题...</string>
<string name="prompt_placeholder_default">在这里输入你的战术指令...</string>

<!-- 对话框 -->
<string name="prompt_discard_title">放弃修改？</string>
<string name="prompt_discard_message">你有未保存的修改，确定要放弃吗？</string>
<string name="prompt_discard_confirm">放弃</string>
<string name="prompt_discard_cancel">继续编辑</string>

<!-- 提示信息 -->
<string name="prompt_save_success">保存成功</string>
<string name="prompt_save_failed">保存失败，请重试</string>
<string name="prompt_load_failed">加载失败</string>
```

**文件位置**：`res/values-en/strings.xml`（英文）

```xml
<!-- Prompt Editor -->
<string name="prompt_editor_title_global">Edit Instruction</string>
<string name="prompt_editor_title_contact">Edit Custom Instruction</string>
<string name="prompt_editor_cancel">Cancel</string>
<string name="prompt_editor_save">Done</string>
<string name="prompt_editor_char_count">Characters: %1$d/%2$d</string>

<!-- Dynamic Placeholders -->
<string name="prompt_placeholder_analyze">e.g., Help me analyze the subtext of her message...</string>
<string name="prompt_placeholder_check">e.g., Pay special attention to work-related topics...</string>
<string name="prompt_placeholder_extract">e.g., Focus on hobbies and interests mentioned...</string>
<string name="prompt_placeholder_summary">e.g., Summarize today\'s conversation highlights...</string>
<string name="prompt_placeholder_contact">e.g., Avoid mentioning ex when chatting with her...</string>
<string name="prompt_placeholder_default">Enter your tactical instruction here...</string>

<!-- Dialog -->
<string name="prompt_discard_title">Discard Changes?</string>
<string name="prompt_discard_message">You have unsaved changes. Are you sure you want to discard them?</string>
<string name="prompt_discard_confirm">Discard</string>
<string name="prompt_discard_cancel">Keep Editing</string>

<!-- Toast Messages -->
<string name="prompt_save_success">Saved successfully</string>
<string name="prompt_save_failed">Save failed, please try again</string>
<string name="prompt_load_failed">Load failed</string>
```

### 2. 代码中使用字符串资源

```kotlin
// 在 Composable 中使用
@Composable
fun PromptEditorTopBar(...) {
    Text(
        text = stringResource(R.string.prompt_editor_cancel),
        ...
    )
}

// 在 ViewModel 中获取占位符
fun getPlaceholderResId(editMode: PromptEditMode): Int {
    return when (editMode) {
        is PromptEditMode.ContactCustom -> R.string.prompt_placeholder_contact
        is PromptEditMode.GlobalScene -> when (editMode.scene) {
            PromptScene.ANALYZE -> R.string.prompt_placeholder_analyze
            PromptScene.CHECK -> R.string.prompt_placeholder_check
            PromptScene.EXTRACT -> R.string.prompt_placeholder_extract
            PromptScene.SUMMARY -> R.string.prompt_placeholder_summary
        }
    }
}
```

---

## ⚡ 性能优化设计

### 1. 大文本输入优化

**问题**：输入超过500字时可能出现卡顿

**解决方案**：

```kotlin
@Composable
fun PromptInputField(...) {
    // 使用 derivedStateOf 减少不必要的重组
    val charCount by remember(value) {
        derivedStateOf { value.length }
    }
    
    // 使用 TextFieldValue 而非 String，避免光标位置丢失
    var textFieldValue by remember {
        mutableStateOf(TextFieldValue(text = value))
    }
    
    // 防抖处理：避免每次输入都触发状态更新
    val debouncedOnValueChange = remember(onValueChange) {
        debounce(100L) { newText: String ->
            onValueChange(newText)
        }
    }
    
    BasicTextField(
        value = textFieldValue,
        onValueChange = { newValue ->
            textFieldValue = newValue
            debouncedOnValueChange(newValue.text)
        },
        ...
    )
}

// 防抖工具函数
fun <T> debounce(
    delayMillis: Long,
    coroutineScope: CoroutineScope,
    action: (T) -> Unit
): (T) -> Unit {
    var debounceJob: Job? = null
    return { param: T ->
        debounceJob?.cancel()
        debounceJob = coroutineScope.launch {
            delay(delayMillis)
            action(param)
        }
    }
}
```

### 2. 内存优化

```kotlin
// 避免在 Composable 中创建大对象
@Composable
fun PromptEditorScreen(...) {
    // ✅ 正确：使用 remember 缓存
    val scrollState = rememberScrollState()
    val bringIntoViewRequester = remember { BringIntoViewRequester() }
    
    // ❌ 错误：每次重组都创建新对象
    // val scrollState = ScrollState(0)
}

// ViewModel 中及时清理资源
@HiltViewModel
class PromptEditorViewModel @Inject constructor(...) : ViewModel() {
    
    override fun onCleared() {
        super.onCleared()
        // 清理可能的资源
    }
}
```

### 3. 动画性能优化

```kotlin
// 使用 animateFloatAsState 而非 animate*AsState 减少重组
val alpha by animateFloatAsState(
    targetValue = if (isNearLimit) 1f else 0.5f,
    animationSpec = tween(durationMillis = 150),
    label = "charCountAlpha"
)

// 避免在动画中使用复杂计算
val charCountColor = remember(isOverLimit, isNearLimit) {
    when {
        isOverLimit -> Color.Red
        isNearLimit -> BrandWarmOrange
        else -> Color.Gray
    }
}
```

### 4. 性能监控指标

| 指标 | 目标值 | 测量方法 |
|------|--------|---------|
| 首次渲染时间 | < 100ms | Compose Metrics |
| 输入响应延迟 | < 16ms | Systrace |
| 内存占用 | < 50MB | Android Profiler |
| 滚动帧率 | 60fps | GPU Profiler |

---

## 🧪 扩展测试用例

### 1. 边界条件测试

```kotlin
@Test
fun `空字符串保存成功`() = runTest {
    val viewModel = createViewModel()
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(""))
    viewModel.onEvent(PromptEditorUiEvent.SavePrompt)
    
    // 空字符串应该可以保存（清除自定义提示词）
    coVerify { promptRepository.saveGlobalPrompt(any(), "") }
}

@Test
fun `恰好1000字符可以保存`() = runTest {
    val viewModel = createViewModel()
    val exactLimitText = "a".repeat(1000)
    
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(exactLimitText))
    
    assertFalse(viewModel.uiState.value.isOverLimit)
    assertTrue(viewModel.uiState.value.canSave)
}

@Test
fun `1001字符不能保存`() = runTest {
    val viewModel = createViewModel()
    val overLimitText = "a".repeat(1001)
    
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(overLimitText))
    
    assertTrue(viewModel.uiState.value.isOverLimit)
    assertFalse(viewModel.uiState.value.canSave)
}

@Test
fun `特殊字符正确处理`() = runTest {
    val viewModel = createViewModel()
    val specialChars = "测试\n换行\t制表符{{变量}}emoji😀"
    
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(specialChars))
    
    assertEquals(specialChars, viewModel.uiState.value.currentPrompt)
}
```

### 2. 性能测试

```kotlin
@Test
fun `长文本输入性能测试`() = runTest {
    val viewModel = createViewModel()
    val longText = "测试文本".repeat(200) // 800字
    
    val startTime = System.currentTimeMillis()
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt(longText))
    val endTime = System.currentTimeMillis()
    
    // 状态更新应在50ms内完成
    assertTrue(endTime - startTime < 50)
}

@Test
fun `快速连续输入不丢失字符`() = runTest {
    val viewModel = createViewModel()
    
    // 模拟快速输入
    repeat(100) { i ->
        viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt("a".repeat(i + 1)))
    }
    
    assertEquals(100, viewModel.uiState.value.charCount)
}
```

### 3. 状态恢复测试

```kotlin
@Test
fun `横竖屏切换时状态保持`() = runTest {
    val savedStateHandle = SavedStateHandle()
    val viewModel = createViewModel(savedStateHandle)
    
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt("测试内容"))
    
    // 模拟配置变更后重建 ViewModel
    val newViewModel = createViewModel(savedStateHandle)
    
    // 状态应该恢复
    assertEquals("测试内容", newViewModel.uiState.value.currentPrompt)
}

@Test
fun `进程被杀后状态恢复`() = runTest {
    val savedStateHandle = SavedStateHandle()
    val viewModel = createViewModel(savedStateHandle)
    
    viewModel.onEvent(PromptEditorUiEvent.UpdatePrompt("重要内容"))
    
    // 模拟进程被杀
    savedStateHandle.set("current_prompt", "重要内容")
    
    val restoredViewModel = createViewModel(savedStateHandle)
    assertEquals("重要内容", restoredViewModel.uiState.value.currentPrompt)
}
```

### 4. 无障碍测试

```kotlin
@Test
fun `输入框有正确的内容描述`() {
    composeTestRule.setContent {
        PromptInputField(
            value = "测试",
            onValueChange = {},
            placeholder = "占位符",
            isLoading = false
        )
    }
    
    composeTestRule.onNodeWithTag("prompt_input")
        .assertContentDescriptionContains("提示词输入框")
}

@Test
fun `字符计数对屏幕阅读器可见`() {
    composeTestRule.setContent {
        CharacterCounter(
            currentCount = 100,
            maxCount = 1000,
            isNearLimit = false,
            isOverLimit = false
        )
    }
    
    composeTestRule.onNodeWithText("字符计数: 100/1000")
        .assertIsDisplayed()
}
```

---

## 📱 版本兼容性

### 支持的 Android 版本

| Android 版本 | API Level | 支持状态 | 备注 |
|-------------|-----------|---------|------|
| Android 7.0+ | 24+ | ✅ 完全支持 | 最低支持版本 |
| Android 10+ | 29+ | ✅ 完全支持 | 推荐版本 |
| Android 12+ | 31+ | ✅ 完全支持 | 最佳体验 |

### 兼容性注意事项

1. **IME Insets**：Android 11+ 原生支持，低版本需要 `WindowCompat` 兼容
2. **Blur Effect**：Android 12+ 原生支持，低版本使用 RenderScript 降级
3. **动态颜色**：Android 12+ Material You 支持，低版本使用固定品牌色

---

## 📅 实施计划

### 阶段划分

| 阶段 | 任务 | 预估工时 | 优先级 |
|------|------|---------|--------|
| 1 | 数据模型和 ViewModel | 2h | P0 |
| 2 | 核心 UI 组件 | 3h | P0 |
| 3 | 导航集成 | 1h | P0 |
| 4 | 键盘避让和光标跟随 | 1h | P0 |
| 5 | 品牌色和视觉细节 | 1h | P1 |
| 6 | 国际化资源 | 0.5h | P1 |
| 7 | 单元测试 | 2h | P1 |
| 8 | UI 测试 | 1h | P2 |
| 9 | 性能优化 | 1h | P2 |

**总预估工时**：12.5h

### 依赖关系

```
阶段1 (数据模型)
    │
    ▼
阶段2 (UI组件) ──────► 阶段5 (视觉细节)
    │                      │
    ▼                      ▼
阶段3 (导航集成)      阶段6 (国际化)
    │
    ▼
阶段4 (键盘避让)
    │
    ▼
阶段7 (单元测试) ──────► 阶段8 (UI测试)
                              │
                              ▼
                         阶段9 (性能优化)
```

---

## 📚 相关文档

- [PRD-00006-提示词编辑界面需求](../PRD/PRD-00006-提示词编辑界面需求.md)
- [FD-00005-提示词管理系统功能设计](FD-00005-提示词管理系统功能设计.md)
- [FD-00004-联系人画像记忆系统UI功能设计](FD-00004-联系人画像记忆系统UI功能设计.md)（参考情感化设计）
- [DR-00012-FD00006文档审查报告](../DR/DR-00012-FD00006文档审查报告.md)
