# FD-00007: 对话上下文连续性增强功能设计

> **文档类型**: 功能设计文档 (FD)  
> **版本**: 1.0  
> **创建日期**: 2025-12-16  
> **最后更新**: 2025-12-16  
> **负责人**: Kiro  
> **状态**: 📝 草稿  
> **优先级**: �  高  
> **关联PRD**: PRD-00007  

---

## 📜 版本历史

| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| 1.0 | 2025-12-16 | Kiro | 初始版本，包含完整架构设计 |
| 1.1 | 2025-12-16 | Kiro | 根据审查意见优化：MessageSender限制说明、本地化建议、SQL性能分析 |

---

## 📋 文档概述

本文档详细描述对话上下文连续性增强功能的技术架构、组件设计、数据流和实现细节。

**设计目标**：
- 让AI能够"记住"最近的对话历史，实现对话连续性
- 通过"时间流逝标记"让AI理解对话的时间间隔
- 模拟人类翻看聊天记录的心理活动，构建自然的上下文结构
- AI能从时间间隔中推断对方可能的情绪变化

**依赖库版本**：
- Room: 2.6.1（数据库）
- Kotlin Coroutines: 1.9.0（异步处理）
- Hilt: 2.52（依赖注入）

---

## 📑 目录

1. [整体架构](#整体架构)
2. [数据模型设计](#数据模型设计)
3. [核心组件设计](#核心组件设计)
4. [时间流逝标记算法](#时间流逝标记算法)
5. [接口设计](#接口设计)
6. [与现有代码的集成设计](#与现有代码的集成设计)
7. [设置界面设计](#设置界面设计)
8. [性能优化设计](#性能优化设计)
9. [错误处理机制](#错误处理机制)
10. [测试策略](#测试策略)

---

## 🏗️ 整体架构

### 架构层次

```
┌─────────────────────────────────────────────────────────┐
│                    Presentation Layer                    │
│  ┌──────────────┐  ┌──────────────┐                    │
│  │SettingsScreen│  │SettingsViewModel│                  │
│  │ (历史条数配置) │  │  (状态管理)   │                    │
│  └──────────────┘  └──────────────┘                    │
└─────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│                     Domain Layer                         │
│  ┌────────────────────┐  ┌────────────────────┐        │
│  │ConversationContext │  │  AnalyzeChatUseCase │        │
│  │     Builder        │  │    (集成历史上下文)  │        │
│  └────────────────────┘  └────────────────────┘        │
│  ┌────────────────────┐  ┌────────────────────┐        │
│  │ TimestampedMessage │  │   TimeFlowMarker   │        │
│  │      (模型)        │  │      (标记类型)     │        │
│  └────────────────────┘  └────────────────────┘        │
└─────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────┐
│                      Data Layer                          │
│  ┌────────────────────┐  ┌────────────────────┐        │
│  │ConversationRepository│ │ ConversationLogDao │        │
│  │       Impl          │  │  (新增查询方法)    │        │
│  └────────────────────┘  └────────────────────┘        │
│  ┌────────────────────┐                                │
│  │SettingsPreferences │                                │
│  │  (历史条数存储)     │                                │
│  └────────────────────┘                                │
└─────────────────────────────────────────────────────────┘
```

### 模块划分

```
domain/
├── model/
│   ├── TimestampedMessage.kt        # 带时间戳的消息模型
│   ├── MessageSender.kt             # 消息发送者枚举
│   └── TimeFlowMarker.kt            # 时间流逝标记密封类
├── util/
│   └── ConversationContextBuilder.kt # 对话上下文构建器
└── repository/
    └── ConversationRepository.kt     # 扩展接口

data/
├── repository/
│   └── ConversationRepositoryImpl.kt # 扩展实现
├── local/
│   ├── dao/
│   │   └── ConversationLogDao.kt     # 扩展DAO
│   └── SettingsPreferences.kt        # 设置存储（扩展）

presentation/
├── ui/screen/settings/
│   └── SettingsScreen.kt             # 设置界面（扩展）
└── viewmodel/
    └── SettingsViewModel.kt          # 设置ViewModel（扩展）
```

---

## 📊 数据模型设计

### 1. MessageSender（消息发送者枚举）

**职责**：标识消息的发送方

> ⚠️ **当前版本限制**：由于数据库 `conversation_logs` 表目前只存储了用户输入（`user_input`），
> 没有存储联系人的消息，因此**当前版本仅回放用户侧历史**。`CONTACT` 类型预留给后续版本扩展。
> 
> **后续扩展方向**：
> - 方案A：扩展数据库表，增加 `sender_type` 字段
> - 方案B：从屏幕抓取时解析消息发送者
> - 方案C：用户手动标记消息归属

```kotlin
/**
 * 消息发送者类型
 * 
 * 【重要说明】当前版本仅支持 USER 类型，CONTACT 类型预留给后续扩展。
 * 原因：数据库 conversation_logs 表目前只存储用户输入，不包含联系人消息。
 */
enum class MessageSender(val prefix: String, val displayName: String) {
    /**
     * 用户发送的消息
     * 【当前版本】：仅此类型被实际使用
     */
    USER("[User]", "我"),
    
    /**
     * 联系人发送的消息
     * 【预留】：后续版本扩展使用
     */
    CONTACT("[Contact]", "对方");
    
    companion object {
        /**
         * 根据前缀解析发送者类型
         */
        fun fromPrefix(prefix: String): MessageSender? {
            return values().find { it.prefix == prefix }
        }
    }
}
```

### 2. TimestampedMessage（带时间戳的消息）

**职责**：承载带时间信息的对话消息

```kotlin
/**
 * 带时间戳的对话消息
 *
 * 用于构建带时间流逝标记的上下文
 *
 * @property content 消息内容
 * @property timestamp 消息时间戳（毫秒）
 * @property sender 消息发送者
 */
data class TimestampedMessage(
    val content: String,
    val timestamp: Long,
    val sender: MessageSender
) {
    init {
        require(content.isNotBlank()) { "消息内容不能为空" }
        require(timestamp > 0) { "时间戳必须大于0" }
    }
    
    /**
     * 获取格式化的时间字符串 (HH:mm)
     */
    fun getFormattedTime(): String {
        val sdf = java.text.SimpleDateFormat("HH:mm", java.util.Locale.getDefault())
        return sdf.format(java.util.Date(timestamp))
    }
    
    /**
     * 获取格式化的日期字符串 (yyyy-MM-dd)
     */
    fun getFormattedDate(): String {
        val sdf = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.getDefault())
        return sdf.format(java.util.Date(timestamp))
    }
    
    /**
     * 转换为显示格式
     * 示例: [User]: 你好 (10:30)
     */
    fun toDisplayString(showTime: Boolean = true): String {
        return if (showTime) {
            "${sender.prefix}: $content (${getFormattedTime()})"
        } else {
            "${sender.prefix}: $content"
        }
    }
}
```

### 3. TimeFlowMarker（时间流逝标记）

**职责**：表示不同类型的时间流逝标记

```kotlin
/**
 * 时间流逝标记类型
 *
 * 用于在对话历史中插入时间间隔提示
 */
sealed class TimeFlowMarker {
    
    /**
     * 日期变更标记
     * 示例: --- [2025-12-16] ---
     *
     * @property date 日期字符串 (yyyy-MM-dd)
     */
    data class DateChange(val date: String) : TimeFlowMarker() {
        override fun toDisplayString(): String = "--- [$date] ---"
    }
    
    /**
     * 短间隔标记（10分钟 ~ 3小时）
     * 示例: --- (经过了 45 分钟) ---
     *
     * @property minutes 间隔分钟数
     * 
     * 【本地化说明】当前硬编码中文字符串，后续版本建议：
     * - 提取到 strings.xml 资源文件
     * - 或使用 android.text.format.DateUtils.getRelativeTimeSpanString()
     */
    data class ShortGap(val minutes: Int) : TimeFlowMarker() {
        override fun toDisplayString(): String {
            // TODO: 后续版本提取到 strings.xml
            // context.getString(R.string.time_flow_short_gap, formatDuration(minutes))
            val duration = if (minutes >= 60) {
                "${minutes / 60} 小时"
            } else {
                "$minutes 分钟"
            }
            return "--- (经过了 $duration) ---"
        }
    }
    
    /**
     * 长间隔标记（> 3小时）
     * 示例: --- (距离上次对话已过 6 小时，注意情绪变化) ---
     *
     * @property hours 间隔小时数
     * 
     * 【本地化说明】当前硬编码中文字符串，后续版本建议提取到资源文件
     */
    data class LongGap(val hours: Int) : TimeFlowMarker() {
        override fun toDisplayString(): String {
            // TODO: 后续版本提取到 strings.xml
            // context.getString(R.string.time_flow_long_gap, hours)
            return "--- (距离上次对话已过 $hours 小时，注意对方可能的情绪堆积) ---"
        }
    }
    
    /**
     * 无标记（热对话，< 10分钟）
     */
    object None : TimeFlowMarker() {
        override fun toDisplayString(): String = ""
    }
    
    /**
     * 转换为显示字符串
     */
    abstract fun toDisplayString(): String
}
```

### 4. ConversationContextConfig（上下文配置）

**职责**：管理对话上下文的配置参数

```kotlin
/**
 * 对话上下文配置
 *
 * @property historyCount 历史对话条数 (0/5/10)
 * @property hotSessionThreshold 热对话阈值（毫秒），默认10分钟
 * @property warmSessionThreshold 温对话阈值（毫秒），默认3小时
 */
data class ConversationContextConfig(
    val historyCount: Int = DEFAULT_HISTORY_COUNT,
    val hotSessionThreshold: Long = DEFAULT_HOT_SESSION_THRESHOLD,
    val warmSessionThreshold: Long = DEFAULT_WARM_SESSION_THRESHOLD
) {
    companion object {
        const val DEFAULT_HISTORY_COUNT = 5
        const val DEFAULT_HOT_SESSION_THRESHOLD = 10 * 60 * 1000L      // 10分钟
        const val DEFAULT_WARM_SESSION_THRESHOLD = 3 * 60 * 60 * 1000L // 3小时
        
        val HISTORY_COUNT_OPTIONS = listOf(0, 5, 10)
    }
    
    init {
        require(historyCount in HISTORY_COUNT_OPTIONS) {
            "历史条数必须是 ${HISTORY_COUNT_OPTIONS.joinToString()} 之一"
        }
        require(hotSessionThreshold > 0) { "热对话阈值必须大于0" }
        require(warmSessionThreshold > hotSessionThreshold) {
            "温对话阈值必须大于热对话阈值"
        }
    }
}
```

---

## 🔧 核心组件设计

### 1. ConversationContextBuilder（对话上下文构建器）

**职责**：构建带时间流逝标记的对话历史上下文

**文件位置**：`domain/util/ConversationContextBuilder.kt`

```kotlin
/**
 * 对话上下文构建器
 *
 * 负责构建带时间流逝标记的对话历史，模拟人类翻看聊天记录的心理活动。
 *
 * 时间流逝标记策略：
 * - < 10分钟：Hot Session，不插入标记
 * - 10分钟 ~ 3小时：Warm Session，插入轻标记
 * - > 3小时：Cold Session，插入强标记并提示情绪变化
 * - 跨天：插入日期标记
 */
@Singleton
class ConversationContextBuilder @Inject constructor() {

    /**
     * 构建带时间流逝标记的对话历史
     *
     * @param messages 按时间正序排列的消息列表
     * @param config 上下文配置
     * @return 格式化后的对话历史字符串
     */
    fun buildHistoryContext(
        messages: List<TimestampedMessage>,
        config: ConversationContextConfig = ConversationContextConfig()
    ): String {
        if (messages.isEmpty()) return ""
        
        return buildString {
            appendLine("【历史对话】(最近${messages.size}条)")
            
            var previousTimestamp: Long? = null
            var previousDate: String? = null
            
            messages.forEach { message ->
                // 计算时间标记
                val marker = calculateTimeMarker(
                    previousTimestamp = previousTimestamp,
                    currentTimestamp = message.timestamp,
                    previousDate = previousDate,
                    config = config
                )
                
                // 插入时间标记（如果有）
                val markerStr = marker.toDisplayString()
                if (markerStr.isNotEmpty()) {
                    appendLine(markerStr)
                }
                
                // 添加消息（在时间间隔后显示时间）
                val showTime = marker !is TimeFlowMarker.None
                appendLine(message.toDisplayString(showTime))
                
                // 更新前一条消息的信息
                previousTimestamp = message.timestamp
                previousDate = message.getFormattedDate()
            }
        }.trimEnd()
    }

    /**
     * 计算时间流逝标记
     *
     * @param previousTimestamp 上一条消息的时间戳
     * @param currentTimestamp 当前消息的时间戳
     * @param previousDate 上一条消息的日期
     * @param config 配置参数
     * @return 时间流逝标记
     */
    fun calculateTimeMarker(
        previousTimestamp: Long?,
        currentTimestamp: Long,
        previousDate: String?,
        config: ConversationContextConfig
    ): TimeFlowMarker {
        // 第一条消息不需要标记
        if (previousTimestamp == null) return TimeFlowMarker.None
        
        val currentDate = formatDate(currentTimestamp)
        
        // 优先检查是否跨天
        if (previousDate != null && previousDate != currentDate) {
            return TimeFlowMarker.DateChange(currentDate)
        }
        
        // 计算时间间隔
        val gap = currentTimestamp - previousTimestamp
        
        return when {
            // 热对话：< 10分钟，不插入标记
            gap < config.hotSessionThreshold -> TimeFlowMarker.None
            
            // 温对话：10分钟 ~ 3小时，插入轻标记
            gap < config.warmSessionThreshold -> {
                val minutes = (gap / (60 * 1000)).toInt()
                TimeFlowMarker.ShortGap(minutes)
            }
            
            // 冷对话：> 3小时，插入强标记
            else -> {
                val hours = (gap / (60 * 60 * 1000)).toInt()
                TimeFlowMarker.LongGap(hours)
            }
        }
    }

    /**
     * 格式化日期
     */
    private fun formatDate(timestamp: Long): String {
        val sdf = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.getDefault())
        return sdf.format(java.util.Date(timestamp))
    }
}
```

### 2. 构建后的上下文示例

**输入数据**：
```kotlin
val messages = listOf(
    TimestampedMessage("宝，早安", 1702684200000, MessageSender.USER),      // 08:10
    TimestampedMessage("早呀", 1702685400000, MessageSender.CONTACT),       // 08:30
    TimestampedMessage("我出门了", 1702686600000, MessageSender.USER),      // 08:50
    TimestampedMessage("路上小心", 1702686900000, MessageSender.CONTACT),   // 08:55
    TimestampedMessage("你今天一天都很忙吗？", 1702708800000, MessageSender.CONTACT) // 15:00
)
```

**输出结果**：
```
【历史对话】(最近5条)
[User]: 宝，早安
[Contact]: 早呀 (08:30)
--- (经过了 20 分钟) ---
[User]: 我出门了 (08:50)
[Contact]: 路上小心 (08:55)
--- (距离上次对话已过 6 小时，注意对方可能的情绪堆积) ---
[Contact]: 你今天一天都很忙吗？ (15:00)
```

---

## ⏱️ 时间流逝标记算法

### 算法流程图

```
┌─────────────────────────────────────────┐
│           开始处理消息                    │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│         是否为第一条消息？                │
│              ↓ 是                        │
│         返回 None                        │
└─────────────────────────────────────────┘
                    ↓ 否
┌─────────────────────────────────────────┐
│           是否跨天？                      │
│              ↓ 是                        │
│    返回 DateChange(当前日期)             │
└─────────────────────────────────────────┘
                    ↓ 否
┌─────────────────────────────────────────┐
│      计算时间间隔 gap                     │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│      gap < 10分钟？                      │
│         ↓ 是                            │
│      返回 None (Hot Session)            │
└─────────────────────────────────────────┘
                    ↓ 否
┌─────────────────────────────────────────┐
│      gap < 3小时？                       │
│         ↓ 是                            │
│   返回 ShortGap(分钟数) (Warm Session)   │
└─────────────────────────────────────────┘
                    ↓ 否
┌─────────────────────────────────────────┐
│   返回 LongGap(小时数) (Cold Session)    │
│   提示：注意对方可能的情绪堆积            │
└─────────────────────────────────────────┘
```

### 时间阈值配置

| 会话状态 | 时间间隔 | 标记类型 | 说明 |
|---------|---------|---------|------|
| Hot Session | < 10分钟 | None | 连贯对话，不插入标记 |
| Warm Session | 10分钟 ~ 3小时 | ShortGap | 同一话题延续，轻标记 |
| Cold Session | > 3小时 | LongGap | 新会话周期，强标记+情绪提示 |
| New Day | 跨天 | DateChange | 日期变更标记 |

### 时间格式化规则

```kotlin
/**
 * 时间间隔格式化
 */
fun formatDuration(minutes: Int): String {
    return when {
        minutes < 60 -> "$minutes 分钟"
        minutes < 120 -> "1 小时"
        else -> "${minutes / 60} 小时"
    }
}
```

---

## 🔌 接口设计

### 1. ConversationRepository 扩展

**新增方法**：获取最近N条对话记录

```kotlin
interface ConversationRepository {
    // ... 现有方法保持不变 ...

    /**
     * 获取指定联系人的最近N条对话记录
     *
     * @param contactId 联系人ID
     * @param limit 最大条数
     * @return 按时间正序排列的对话记录（最早的在前）
     */
    suspend fun getRecentConversations(
        contactId: String,
        limit: Int
    ): Result<List<ConversationLog>>
}
```

### 2. ConversationRepositoryImpl 实现

```kotlin
@Singleton
class ConversationRepositoryImpl @Inject constructor(
    private val dao: ConversationLogDao
) : ConversationRepository {

    // ... 现有方法保持不变 ...

    override suspend fun getRecentConversations(
        contactId: String,
        limit: Int
    ): Result<List<ConversationLog>> = withContext(Dispatchers.IO) {
        try {
            val entities = dao.getRecentConversations(contactId, limit)
            Result.success(entities.map { it.toDomain() })
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
}
```

### 3. ConversationLogDao 扩展

```kotlin
@Dao
interface ConversationLogDao {
    // ... 现有方法保持不变 ...

    /**
     * 获取指定联系人的最近N条对话记录
     * 按时间戳正序排列（最早的在前）
     *
     * 使用子查询先获取最近N条（倒序），再正序排列
     * 
     * 【SQL性能分析】
     * - 利用 contact_id 索引快速定位联系人记录
     * - 利用 timestamp 索引进行排序
     * - 子查询 + LIMIT 避免全表扫描
     * - 预估性能：单人App场景（<10万条记录），查询时间 < 10ms
     * - 极端场景（>50万条记录）可能需要复合索引优化
     */
    @Query("""
        SELECT * FROM (
            SELECT * FROM conversation_logs 
            WHERE contact_id = :contactId 
            ORDER BY timestamp DESC 
            LIMIT :limit
        ) 
        ORDER BY timestamp ASC
    """)
    suspend fun getRecentConversations(contactId: String, limit: Int): List<ConversationLogEntity>
}
```

> 💡 **性能优化建议（后续版本）**：
> 如果数据量增长到极端规模，可考虑添加复合索引：
> ```sql
> CREATE INDEX idx_contact_timestamp ON conversation_logs(contact_id, timestamp DESC)
> ```

### 4. SettingsRepository 扩展

```kotlin
interface SettingsRepository {
    // ... 现有方法保持不变 ...

    /**
     * 获取历史对话条数配置
     * @return 0/5/10，默认5
     */
    suspend fun getHistoryConversationCount(): Result<Int>

    /**
     * 设置历史对话条数
     * @param count 条数，必须是 0/5/10 之一
     */
    suspend fun setHistoryConversationCount(count: Int): Result<Unit>
}
```

### 5. SettingsRepositoryImpl 实现

```kotlin
@Singleton
class SettingsRepositoryImpl @Inject constructor(
    private val settingsPreferences: SettingsPreferences
) : SettingsRepository {

    // ... 现有方法保持不变 ...

    override suspend fun getHistoryConversationCount(): Result<Int> {
        return try {
            Result.success(settingsPreferences.getHistoryConversationCount())
        } catch (e: Exception) {
            Result.failure(e)
        }
    }

    override suspend fun setHistoryConversationCount(count: Int): Result<Unit> {
        return try {
            require(count in ConversationContextConfig.HISTORY_COUNT_OPTIONS) {
                "历史条数必须是 ${ConversationContextConfig.HISTORY_COUNT_OPTIONS.joinToString()} 之一"
            }
            settingsPreferences.setHistoryConversationCount(count)
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
}
```

### 6. SettingsPreferences 扩展

```kotlin
@Singleton
class SettingsPreferences @Inject constructor(
    @ApplicationContext private val context: Context
) {
    private val prefs = context.getSharedPreferences("settings", Context.MODE_PRIVATE)

    companion object {
        private const val KEY_HISTORY_CONVERSATION_COUNT = "history_conversation_count"
        private const val DEFAULT_HISTORY_COUNT = 5
    }

    // ... 现有方法保持不变 ...

    /**
     * 获取历史对话条数
     */
    fun getHistoryConversationCount(): Int {
        return prefs.getInt(KEY_HISTORY_CONVERSATION_COUNT, DEFAULT_HISTORY_COUNT)
    }

    /**
     * 设置历史对话条数
     */
    fun setHistoryConversationCount(count: Int) {
        prefs.edit().putInt(KEY_HISTORY_CONVERSATION_COUNT, count).apply()
    }
}
```

---

## 🔗 与现有代码的集成设计

### 1. AnalyzeChatUseCase 集成

**修改位置**：`domain/usecase/AnalyzeChatUseCase.kt`

**集成方案**：在构建上下文时加入历史对话

```kotlin
class AnalyzeChatUseCase @Inject constructor(
    private val contactRepository: ContactRepository,
    private val brainTagRepository: BrainTagRepository,
    private val privacyRepository: PrivacyRepository,
    private val aiRepository: AiRepository,
    private val settingsRepository: SettingsRepository,
    private val aiProviderRepository: AiProviderRepository,
    private val conversationRepository: ConversationRepository,
    private val promptBuilder: PromptBuilder,
    private val conversationContextBuilder: ConversationContextBuilder  // 新增
) {
    suspend operator fun invoke(
        contactId: String,
        rawScreenContext: List<String>
    ): Result<AnalysisResult> {
        // ... 现有前置检查逻辑 ...

        // 【新增】获取历史对话配置
        val historyCount = settingsRepository.getHistoryConversationCount()
            .getOrDefault(ConversationContextConfig.DEFAULT_HISTORY_COUNT)

        // 【新增】构建历史上下文
        val historyContext = if (historyCount > 0) {
            buildHistoryContext(contactId, historyCount)
        } else {
            ""
        }

        // 构建运行时数据（加入历史上下文）
        val runtimeData = buildContextData(
            targetGoal = profile.targetGoal,
            facts = profile.facts,
            redTags = redTags,
            greenTags = greenTags,
            conversationHistory = maskedContext,
            historyContext = historyContext  // 新增参数
        )

        // ... 后续AI调用逻辑 ...
    }

    /**
     * 【新增】构建历史上下文
     * 
     * 【重要说明】当前版本仅回放用户侧历史
     * 原因：数据库 conversation_logs 表只存储了 user_input，
     * 没有存储联系人的消息，因此 sender 固定为 MessageSender.USER。
     * 
     * 后续扩展方向：
     * - 扩展数据库表增加 sender_type 字段
     * - 或从屏幕抓取时解析消息发送者
     */
    private suspend fun buildHistoryContext(contactId: String, limit: Int): String {
        val recentLogs = conversationRepository
            .getRecentConversations(contactId, limit)
            .getOrDefault(emptyList())

        if (recentLogs.isEmpty()) return ""

        // 将ConversationLog转换为TimestampedMessage
        // 【注意】当前版本 sender 固定为 USER，因为数据库只存储用户输入
        val messages = recentLogs.map { log ->
            TimestampedMessage(
                content = log.userInput,
                timestamp = log.timestamp,
                sender = MessageSender.USER  // 当前版本固定为USER
            )
        }

        return conversationContextBuilder.buildHistoryContext(messages)
    }

    /**
     * 【修改】构建上下文数据，加入历史对话区块
     */
    private fun buildContextData(
        targetGoal: String,
        facts: List<Fact>,
        redTags: List<BrainTag>,
        greenTags: List<BrainTag>,
        conversationHistory: List<String>,
        historyContext: String = ""  // 新增参数
    ): String {
        return buildString {
            // 【新增】历史对话区块（放在最前面，让AI先了解背景）
            if (historyContext.isNotBlank()) {
                appendLine(historyContext)
                appendLine()
            }

            appendLine("【攻略目标】")
            appendLine(targetGoal)
            appendLine()

            if (facts.isNotEmpty()) {
                appendLine("【已知信息】")
                facts.forEach { fact ->
                    appendLine("- ${fact.key}: ${fact.value}")
                }
                appendLine()
            }

            if (redTags.isNotEmpty()) {
                appendLine("【雷区警告】")
                redTags.forEach { tag ->
                    appendLine("- ${tag.content}")
                }
                appendLine()
            }

            if (greenTags.isNotEmpty()) {
                appendLine("【策略建议】")
                greenTags.forEach { tag ->
                    appendLine("- ${tag.content}")
                }
                appendLine()
            }

            appendLine("【当前聊天记录】")
            conversationHistory.forEach { message ->
                appendLine(message)
            }
        }
    }
}
```

### 2. 完整上下文结构示例

**AI最终看到的上下文**：

```
【历史对话】(最近5条)
[User]: 宝，早安
[Contact]: 早呀 (08:30)
--- (经过了 20 分钟) ---
[User]: 我出门了 (08:50)
[Contact]: 路上小心 (08:55)
--- (距离上次对话已过 6 小时，注意对方可能的情绪堆积) ---
[Contact]: 你今天一天都很忙吗？ (15:00)

【攻略目标】
追求阶段，希望发展成恋人关系

【已知信息】
- 生日: 3月15日
- 爱好: 看电影、喝咖啡
- 工作: 设计师

【雷区警告】
- 不要提前任
- 不要催促回复

【策略建议】
- 多关心她的工作
- 适当表达关心

【当前聊天记录】
你今天一天都很忙吗？
```

---

## 🖥️ 设置界面设计

### 1. 设置页面UI扩展

**位置**：设置页面 → AI配置区域

```
┌─────────────────────────────────────────┐
│ AI 配置                                  │
├─────────────────────────────────────────┤
│ 默认服务商              DeepSeek    >   │
├─────────────────────────────────────────┤
│ 提示词管理                          >   │
├─────────────────────────────────────────┤
│ 历史对话条数                            │
│ ┌─────────────────────────────────────┐ │
│ │ ○ 不发送历史 (0条)                  │ │
│ │ ● 最近5条 (推荐)                    │ │
│ │ ○ 最近10条                          │ │
│ └─────────────────────────────────────┘ │
│ 发送给AI的历史对话数量，帮助AI理解      │
│ 对话上下文和时间间隔                    │
└─────────────────────────────────────────┘
```

### 2. SettingsUiState 扩展

```kotlin
data class SettingsUiState(
    // ... 现有字段 ...
    
    /**
     * 历史对话条数配置
     */
    val historyConversationCount: Int = 5,
    
    /**
     * 历史条数选项
     */
    val historyCountOptions: List<HistoryCountOption> = listOf(
        HistoryCountOption(0, "不发送历史", "每次分析独立，不包含历史对话"),
        HistoryCountOption(5, "最近5条", "推荐，平衡上下文和Token消耗"),
        HistoryCountOption(10, "最近10条", "更完整的上下文，Token消耗较高")
    )
)

/**
 * 历史条数选项
 */
data class HistoryCountOption(
    val count: Int,
    val label: String,
    val description: String
)
```

### 3. SettingsUiEvent 扩展

```kotlin
sealed class SettingsUiEvent {
    // ... 现有事件 ...
    
    /**
     * 更改历史对话条数
     */
    data class ChangeHistoryConversationCount(val count: Int) : SettingsUiEvent()
}
```

### 4. SettingsViewModel 扩展

```kotlin
@HiltViewModel
class SettingsViewModel @Inject constructor(
    // ... 现有依赖 ...
    private val settingsRepository: SettingsRepository
) : ViewModel() {

    // ... 现有代码 ...

    init {
        loadSettings()
    }

    private fun loadSettings() {
        viewModelScope.launch {
            // ... 现有加载逻辑 ...
            
            // 加载历史对话条数配置
            val historyCount = settingsRepository.getHistoryConversationCount()
                .getOrDefault(5)
            _uiState.update { it.copy(historyConversationCount = historyCount) }
        }
    }

    fun onEvent(event: SettingsUiEvent) {
        when (event) {
            // ... 现有事件处理 ...
            
            is SettingsUiEvent.ChangeHistoryConversationCount -> {
                changeHistoryConversationCount(event.count)
            }
        }
    }

    private fun changeHistoryConversationCount(count: Int) {
        viewModelScope.launch {
            settingsRepository.setHistoryConversationCount(count)
                .onSuccess {
                    _uiState.update { it.copy(historyConversationCount = count) }
                }
                .onFailure { e ->
                    // 显示错误提示
                    Log.e(TAG, "保存历史条数配置失败", e)
                }
        }
    }
}
```

### 5. SettingsScreen UI组件

```kotlin
@Composable
fun HistoryConversationCountSection(
    selectedCount: Int,
    options: List<HistoryCountOption>,
    onCountChange: (Int) -> Unit,
    modifier: Modifier = Modifier
) {
    Column(modifier = modifier.padding(16.dp)) {
        Text(
            text = "历史对话条数",
            style = MaterialTheme.typography.titleMedium
        )
        
        Spacer(modifier = Modifier.height(8.dp))
        
        options.forEach { option ->
            Row(
                modifier = Modifier
                    .fillMaxWidth()
                    .clickable { onCountChange(option.count) }
                    .padding(vertical = 8.dp),
                verticalAlignment = Alignment.CenterVertically
            ) {
                RadioButton(
                    selected = selectedCount == option.count,
                    onClick = { onCountChange(option.count) }
                )
                
                Spacer(modifier = Modifier.width(8.dp))
                
                Column {
                    Text(
                        text = option.label,
                        style = MaterialTheme.typography.bodyLarge
                    )
                    Text(
                        text = option.description,
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
            }
        }
        
        Spacer(modifier = Modifier.height(8.dp))
        
        Text(
            text = "发送给AI的历史对话数量，帮助AI理解对话上下文和时间间隔",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant
        )
    }
}
```

---

## ⚡ 性能优化设计

### 1. 数据库查询优化

**索引利用**：`conversation_logs` 表已有 `contact_id` 和 `timestamp` 索引

```kotlin
// 现有索引定义（无需修改）
@Entity(
    tableName = "conversation_logs",
    indices = [
        Index(value = ["contact_id"]),
        Index(value = ["timestamp"]),
        Index(value = ["is_summarized"])
    ]
)
```

**查询优化**：使用子查询避免全表扫描

```sql
-- 优化后的查询（先倒序取N条，再正序排列）
SELECT * FROM (
    SELECT * FROM conversation_logs 
    WHERE contact_id = :contactId 
    ORDER BY timestamp DESC 
    LIMIT :limit
) 
ORDER BY timestamp ASC
```

### 2. 内存优化

- **限制历史条数**：最多10条，避免内存占用过大
- **字符串构建**：使用 `buildString` 避免多次字符串拼接
- **懒加载**：只在需要时才查询历史对话

### 3. Token消耗估算

| 历史条数 | 预估Token增加 | 说明 |
|---------|--------------|------|
| 0条 | 0 | 不发送历史 |
| 5条 | ~200-400 | 约5-10%增加 |
| 10条 | ~400-800 | 约10-20%增加 |

**优化策略**：时间标记简洁，不给每条消息都加标记

---

## 🚨 错误处理机制

### 1. 错误类型定义

```kotlin
/**
 * 对话上下文构建错误
 */
sealed class ConversationContextError : Exception() {
    /**
     * 数据库查询失败
     */
    data class DatabaseError(override val cause: Throwable) : ConversationContextError()
    
    /**
     * 配置无效
     */
    data class InvalidConfig(override val message: String) : ConversationContextError()
}
```

### 2. 错误处理策略

| 错误场景 | 处理策略 | 用户影响 |
|---------|---------|---------|
| 数据库查询失败 | 返回空历史，继续分析 | 无感知，分析正常进行 |
| 配置读取失败 | 使用默认值(5条) | 无感知 |
| 时间戳异常 | 跳过该条消息 | 历史可能不完整 |

### 3. 降级策略

```kotlin
private suspend fun buildHistoryContext(contactId: String, limit: Int): String {
    return try {
        val recentLogs = conversationRepository
            .getRecentConversations(contactId, limit)
            .getOrDefault(emptyList())

        if (recentLogs.isEmpty()) return ""

        val messages = recentLogs.mapNotNull { log ->
            try {
                TimestampedMessage(
                    content = log.userInput,
                    timestamp = log.timestamp,
                    sender = MessageSender.USER
                )
            } catch (e: Exception) {
                Log.w(TAG, "跳过无效的对话记录: ${log.id}", e)
                null  // 跳过无效记录
            }
        }

        conversationContextBuilder.buildHistoryContext(messages)
    } catch (e: Exception) {
        Log.e(TAG, "构建历史上下文失败，降级为空历史", e)
        ""  // 降级：返回空历史，不影响主流程
    }
}
```

---

## 🧪 测试策略

### 1. 单元测试

#### ConversationContextBuilderTest

```kotlin
class ConversationContextBuilderTest {

    private lateinit var builder: ConversationContextBuilder

    @Before
    fun setup() {
        builder = ConversationContextBuilder()
    }

    @Test
    fun `空消息列表返回空字符串`() {
        val result = builder.buildHistoryContext(emptyList())
        assertEquals("", result)
    }

    @Test
    fun `单条消息不插入时间标记`() {
        val messages = listOf(
            TimestampedMessage("你好", 1702684200000, MessageSender.USER)
        )
        val result = builder.buildHistoryContext(messages)
        
        assertTrue(result.contains("[User]: 你好"))
        assertFalse(result.contains("---"))
    }

    @Test
    fun `热对话不插入时间标记`() {
        val baseTime = 1702684200000L
        val messages = listOf(
            TimestampedMessage("你好", baseTime, MessageSender.USER),
            TimestampedMessage("你好呀", baseTime + 5 * 60 * 1000, MessageSender.CONTACT) // 5分钟后
        )
        val result = builder.buildHistoryContext(messages)
        
        assertFalse(result.contains("经过了"))
    }

    @Test
    fun `温对话插入轻标记`() {
        val baseTime = 1702684200000L
        val messages = listOf(
            TimestampedMessage("你好", baseTime, MessageSender.USER),
            TimestampedMessage("你好呀", baseTime + 30 * 60 * 1000, MessageSender.CONTACT) // 30分钟后
        )
        val result = builder.buildHistoryContext(messages)
        
        assertTrue(result.contains("经过了 30 分钟"))
    }

    @Test
    fun `冷对话插入强标记并提示情绪`() {
        val baseTime = 1702684200000L
        val messages = listOf(
            TimestampedMessage("你好", baseTime, MessageSender.USER),
            TimestampedMessage("你好呀", baseTime + 6 * 60 * 60 * 1000, MessageSender.CONTACT) // 6小时后
        )
        val result = builder.buildHistoryContext(messages)
        
        assertTrue(result.contains("距离上次对话已过 6 小时"))
        assertTrue(result.contains("情绪堆积"))
    }

    @Test
    fun `跨天插入日期标记`() {
        val day1 = 1702684200000L  // 2023-12-16 08:10
        val day2 = day1 + 24 * 60 * 60 * 1000  // 2023-12-17 08:10
        val messages = listOf(
            TimestampedMessage("晚安", day1, MessageSender.USER),
            TimestampedMessage("早安", day2, MessageSender.CONTACT)
        )
        val result = builder.buildHistoryContext(messages)
        
        assertTrue(result.contains("--- ["))
    }

    @Test
    fun `时间格式化正确`() {
        val messages = listOf(
            TimestampedMessage("你好", 1702684200000, MessageSender.USER) // 08:10
        )
        val result = builder.buildHistoryContext(messages)
        
        // 第一条消息不显示时间（因为没有时间间隔）
        assertTrue(result.contains("[User]: 你好"))
    }
}
```

#### TimeFlowMarkerTest

```kotlin
class TimeFlowMarkerTest {

    @Test
    fun `DateChange显示格式正确`() {
        val marker = TimeFlowMarker.DateChange("2025-12-16")
        assertEquals("--- [2025-12-16] ---", marker.toDisplayString())
    }

    @Test
    fun `ShortGap分钟显示格式正确`() {
        val marker = TimeFlowMarker.ShortGap(30)
        assertEquals("--- (经过了 30 分钟) ---", marker.toDisplayString())
    }

    @Test
    fun `ShortGap小时显示格式正确`() {
        val marker = TimeFlowMarker.ShortGap(90)
        assertEquals("--- (经过了 1 小时) ---", marker.toDisplayString())
    }

    @Test
    fun `LongGap显示格式正确`() {
        val marker = TimeFlowMarker.LongGap(6)
        assertEquals(
            "--- (距离上次对话已过 6 小时，注意对方可能的情绪堆积) ---",
            marker.toDisplayString()
        )
    }

    @Test
    fun `None返回空字符串`() {
        assertEquals("", TimeFlowMarker.None.toDisplayString())
    }
}
```

#### TimestampedMessageTest

```kotlin
class TimestampedMessageTest {

    @Test
    fun `toDisplayString包含发送者前缀`() {
        val message = TimestampedMessage("你好", 1702684200000, MessageSender.USER)
        assertTrue(message.toDisplayString().startsWith("[User]:"))
    }

    @Test
    fun `toDisplayString包含时间`() {
        val message = TimestampedMessage("你好", 1702684200000, MessageSender.USER)
        val result = message.toDisplayString(showTime = true)
        assertTrue(result.contains("("))
        assertTrue(result.contains(")"))
    }

    @Test
    fun `toDisplayString不包含时间`() {
        val message = TimestampedMessage("你好", 1702684200000, MessageSender.USER)
        val result = message.toDisplayString(showTime = false)
        assertFalse(result.contains("("))
    }

    @Test(expected = IllegalArgumentException::class)
    fun `空内容抛出异常`() {
        TimestampedMessage("", 1702684200000, MessageSender.USER)
    }

    @Test(expected = IllegalArgumentException::class)
    fun `无效时间戳抛出异常`() {
        TimestampedMessage("你好", 0, MessageSender.USER)
    }
}
```

### 2. 集成测试

#### AnalyzeChatUseCaseIntegrationTest

```kotlin
@HiltAndroidTest
class AnalyzeChatUseCaseIntegrationTest {

    @Inject
    lateinit var analyzeChatUseCase: AnalyzeChatUseCase

    @Inject
    lateinit var conversationRepository: ConversationRepository

    @Inject
    lateinit var settingsRepository: SettingsRepository

    @Test
    fun `分析时包含历史对话上下文`() = runTest {
        // Given: 设置历史条数为5
        settingsRepository.setHistoryConversationCount(5)
        
        // Given: 保存一些历史对话
        conversationRepository.saveUserInput("contact_1", "早安")
        conversationRepository.saveUserInput("contact_1", "我出门了")
        
        // When: 执行分析
        val result = analyzeChatUseCase(
            contactId = "contact_1",
            rawScreenContext = listOf("你今天忙吗？")
        )
        
        // Then: 验证历史上下文被包含
        // (通过日志或Mock验证)
    }

    @Test
    fun `历史条数为0时不包含历史`() = runTest {
        // Given: 设置历史条数为0
        settingsRepository.setHistoryConversationCount(0)
        
        // When: 执行分析
        val result = analyzeChatUseCase(
            contactId = "contact_1",
            rawScreenContext = listOf("你好")
        )
        
        // Then: 验证没有历史上下文
    }
}
```

### 3. 测试覆盖目标

| 模块 | 目标覆盖率 | 重点测试项 |
|------|-----------|-----------|
| ConversationContextBuilder | 90%+ | 时间标记算法、边界条件 |
| TimeFlowMarker | 100% | 所有标记类型的显示格式 |
| TimestampedMessage | 100% | 格式化、验证逻辑 |
| ConversationRepository扩展 | 80%+ | 查询正确性、排序 |
| SettingsRepository扩展 | 80%+ | 配置读写 |

---

## 📋 实施计划

### 阶段1：领域层实现（预计1天）

| 任务 | 文件 | 说明 |
|------|------|------|
| 1.1 | `MessageSender.kt` | 新建消息发送者枚举 |
| 1.2 | `TimestampedMessage.kt` | 新建带时间戳消息模型 |
| 1.3 | `TimeFlowMarker.kt` | 新建时间流逝标记密封类 |
| 1.4 | `ConversationContextConfig.kt` | 新建配置模型 |
| 1.5 | `ConversationContextBuilder.kt` | 新建上下文构建器 |
| 1.6 | 单元测试 | 编写上述组件的单元测试 |

### 阶段2：数据层实现（预计0.5天）

| 任务 | 文件 | 说明 |
|------|------|------|
| 2.1 | `ConversationLogDao.kt` | 添加 `getRecentConversations` 方法 |
| 2.2 | `ConversationRepository.kt` | 扩展接口 |
| 2.3 | `ConversationRepositoryImpl.kt` | 实现新方法 |
| 2.4 | `SettingsPreferences.kt` | 添加历史条数存储 |
| 2.5 | `SettingsRepository.kt` | 扩展接口 |
| 2.6 | `SettingsRepositoryImpl.kt` | 实现新方法 |

### 阶段3：业务层集成（预计0.5天）

| 任务 | 文件 | 说明 |
|------|------|------|
| 3.1 | `AnalyzeChatUseCase.kt` | 集成历史上下文构建 |
| 3.2 | `ContextBuilder.kt` | 可选：扩展支持历史区块 |

### 阶段4：UI层实现（预计0.5天）

| 任务 | 文件 | 说明 |
|------|------|------|
| 4.1 | `SettingsUiState.kt` | 添加历史条数状态 |
| 4.2 | `SettingsUiEvent.kt` | 添加更改事件 |
| 4.3 | `SettingsViewModel.kt` | 实现事件处理 |
| 4.4 | `SettingsScreen.kt` | 添加UI组件 |

### 阶段5：测试与验收（预计1天）

| 任务 | 说明 |
|------|------|
| 5.1 | 补充集成测试 |
| 5.2 | 端到端测试 |
| 5.3 | 性能测试 |
| 5.4 | 文档更新 |

**总预计工作量**：3.5天

---

## 📚 相关文档

- [PRD-00007-对话上下文连续性增强需求](../PRD/PRD-00007-对话上下文连续性增强需求.md)
- [ContextBuilder.kt](../../../app/src/main/java/com/empathy/ai/domain/util/ContextBuilder.kt)
- [ConversationRepository.kt](../../../app/src/main/java/com/empathy/ai/domain/repository/ConversationRepository.kt)
- [AnalyzeChatUseCase.kt](../../../app/src/main/java/com/empathy/ai/domain/usecase/AnalyzeChatUseCase.kt)
- [ConversationLogDao.kt](../../../app/src/main/java/com/empathy/ai/data/local/dao/ConversationLogDao.kt)
