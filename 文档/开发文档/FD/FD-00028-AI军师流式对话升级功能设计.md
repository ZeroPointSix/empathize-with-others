...
在现有AI军师对话功能（FD-00026）基础上，升级为流式对话体验，参考Cherry Studio的优秀设计实现：

1. **流式响应**：AI回复实时逐字显示，提升交互体验
2. **思考过程展示**：支持DeepSeek R1等模型的思考过程可视化
3. **Block架构**：消息拆分为独立Block，支持多种内容类型
4. **请求控制**：支持停止生成、重新生成等操作

### 1.2 核心功能点

| 功能点 | 描述 | 优先级 |
|-------|------|--------|
| SSE流式响应 | 使用Server-Sent Events实现实时文本流 | P0 |
| 思考过程展示 | 可折叠的思考过程区域，显示AI推理过程 | P0 |
| Block架构 | 消息拆分为THINKING/MAIN_TEXT等Block | P0 |
| 停止生成 | 用户可随时中断AI响应 | P1 |
| 重新生成 | 重新生成最后一条AI回复 | P1 |
| 智能节流 | 减少数据库写入频率，提升性能 | P1 |
| 降级策略 | SSE失败时自动降级到非流式模式 | P2 |

### 1.3 与现有功能的区别

| 维度 | FD-00026 (现有) | FD-00028 (升级) |
|------|----------------|-----------------|
| 响应方式 | 一次性返回完整内容 | 流式逐字显示 |
| 思考过程 | 不支持 | 支持展示和折叠 |
| 数据结构 | 单一content字段 | Block架构（多块） |
| 用户控制 | 无 | 停止/重新生成 |
| 数据库写入 | 完成后一次写入 | 智能节流批量写入 |
...
### 2.1 模块分布（遵循Clean Architecture）

```
┌─────────────────────────────────────────────────────────────┐
│                      :app 模块                              │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ AiAdvisorModule.kt (DI模块扩展)                     │   │
│  │ - 流式UseCase注入                                   │   │
│  │ - BlockUpdateManager注入                            │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                              │
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────────┐
│   :domain 模块   │ │   :data 模块    │ │  :presentation 模块  │
│                 │ │                 │ │                     │
│ 🆕 AiStreamChunk │ │ 🆕 SseStreamReader │ │ 🆕 ThinkingSection  │
│ 🆕 MessageBlockType │ │ 🆕 BlockUpdateManager │ │ 🆕 StreamingBubble │
│ 🆕 MessageBlockStatus │ │ 🆕 AiAdvisorMessageBlockDao │ │ 🆕 StopButton    │
│ 🆕 AiAdvisorMessageBlock │ │ 🆕 AiAdvisorMessageBlockEntity │ │                │
│ 🆕 StreamingState │ │                 │ │                     │
...
| 模块 | 新增组件 | 职责 |
|------|---------|------|
| :domain | AiStreamChunk, MessageBlockType, MessageBlockStatus, AiAdvisorMessageBlock, StreamingState | 流式数据模型定义，纯Kotlin无Android依赖 |
| :data | SseStreamReader, BlockUpdateManager, AiAdvisorMessageBlockDao, AiAdvisorMessageBlockEntity | SSE流式读取、智能节流、数据库操作 |
| :presentation | ThinkingSection, StreamingMessageBubble, StopGenerationButton | 流式UI组件、思考过程展示 |
| :app | AiAdvisorModule扩展 | 流式相关依赖注入配置 |
...
│ └── 4. 调用SendAdvisorMessageStreamingUseCase               │
└─────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────┐
│ SendAdvisorMessageStreamingUseCase                          │
│ ├── 1. 构建AI军师提示词                                     │
│ ├── 2. 调用AiRepository.generateTextStream()                │
│ └── 3. 返回Flow<StreamingState>                             │
└─────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────┐
│ SseStreamReader (SSE流式读取)                               │
│ ├── 解析SSE事件                                             │
│ ├── 识别思考内容(reasoning_content)                         │
│ └── 发射AiStreamChunk                                       │
└─────────────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────┐
│ BlockUpdateManager (智能节流)                               │
│ ├── 块类型变化 → 立即写入                                   │
│ ├── 同类型内容 → 300ms节流                                  │
│ └── 完成时 → 强制刷新                                       │
└─────────────────────────────────────────────────────────────┘
 ...
 * 注意：Throwable是Kotlin标准库类，在:domain模块中可以使用。
 */
...
#### 3.1.2 MessageBlockType/MessageBlockStatus（消息块类型和状态）
...
/**
 * 消息块类型
 * 参考Cherry Studio的MessageBlockType设计
 */
enum class MessageBlockType {
    MAIN_TEXT,    // 主文本
    THINKING,     // 思考过程
    ERROR         // 错误信息
    // 未来扩展：CODE, TOOL, CITATION
}

/**
 * 消息块状态
 * 参考Cherry Studio的MessageBlockStatus设计
 */
...
#### 3.1.3 AiAdvisorMessageBlock（消息块模型）
...
data class BlockMetadata(
    val thinkingMs: Long? = null,    // 思考耗时（毫秒）
    val tokenCount: Int? = null      // Token数量
)
```

#### 3.1.4 StreamingState（流式状态）
...
#### 3.1.5 SendStatus枚举扩展

```kotlin
// domain/src/main/kotlin/com/empathy/ai/domain/model/SendStatus.kt

/**
 * 消息发送状态
 * 🆕 新增CANCELLED状态支持用户取消操作
 */
enum class SendStatus {
    PENDING,    // 等待发送
    SUCCESS,    // 发送成功
    FAILED,     // 发送失败
    CANCELLED   // 🆕 用户取消（流式响应被中断）
}
```
...
@Database(
    entities = [
        ...
        AiAdvisorMessageBlockEntity::class  // 🆕 新增
    ],
    version = 14,  // 🆕 版本升级
    exportSchema = true
)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    abstract fun aiAdvisorMessageBlockDao(): AiAdvisorMessageBlockDao  // 🆕 新增
}
```
...
### 4.2 AiAdvisorRepository扩展（:domain模块）
...
### 4.3 AiAdvisorMessageBlockDao（:data模块）
...
### 5.1 SendAdvisorMessageStreamingUseCase（核心流式用例）
...
class SendAdvisorMessageStreamingUseCase @Inject constructor(
    ...
    companion object {
         ...
         * 系统提示词
         * 
         * 注：完整版提示词定义在SystemPrompts.AI_ADVISOR_HEADER中，
         * 此处使用简化版本用于文档示例。实际实现应引用SystemPrompts。
         * 
         * @see SystemPrompts.AI_ADVISOR_HEADER
         * @see SystemPrompts.AI_ADVISOR_FOOTER
         */
        private const val SYSTEM_INSTRUCTION = """
你是一位专业的社交沟通顾问，帮助用户分析聊天场景并提供建议。
                ...
                override fun onFailure(
                    ...
                    if (consecutiveFailures >= FALLBACK_THRESHOLD) {
                        trySend(AiStreamChunk.Error(
                            SseFallbackException("连续${consecutiveFailures}次SSE失败", error)
                        ))
                    } else {
...
### 6.2 BlockUpdateManager（智能节流更新管理器）
...
| 服务商 | 文本字段 | 思考过程字段 | 结束标记 | 支持状态 |
|--------|---------|-------------|---------|---------|
| OpenAI | `delta.content` | 不支持 | `[DONE]` | ✅ 支持 |
| DeepSeek Chat | `delta.content` | 不支持 | `[DONE]` | ✅ 支持 |
| DeepSeek R1 | `delta.content` | `delta.reasoning_content` | `[DONE]` | ✅ 支持 |
| Claude | `delta.text` | 不支持 | `event: message_stop` | ⏳ 待适配 |

### 6.4 降级策略

```
SSE请求 → 失败 → 重试(最多3次) → 连续失败3次 → 降级到非流式
                    ↓
                  成功 → 重置失败计数
```

**降级触发条件**：连续3次SSE连接失败
**降级后行为**：使用`SendAdvisorMessageUseCase`（非流式版本）
...
### 7.1 流式消息气泡（StreamingMessageBubble）
...
### 7.3 错误展示组件（ErrorSection）

```kotlin
// presentation/src/main/kotlin/com/empathy/ai/presentation/ui/screen/advisor/component/ErrorSection.kt
...
### 7.4 停止生成按钮（StopGenerationButton）
...
class AiAdvisorChatViewModel @Inject constructor(
    ...
    private val sendAdvisorMessageUseCase: SendAdvisorMessageUseCase,  // 非流式备用
...
| 测试类 | 测试内容 | 模块 |
|--------|---------|------|
| `AiStreamChunkTest` | 流式数据块模型测试 | :domain |
| `MessageBlockTypeTest` | 消息块类型枚举测试 | :domain |
| `AiAdvisorMessageBlockTest` | 消息块模型测试 | :domain |
| `SendAdvisorMessageStreamingUseCaseTest` | 流式发送用例测试 | :domain |
| `SseStreamReaderTest` | SSE流式读取器测试 | :data |
| `BlockUpdateManagerTest` | 智能节流管理器测试 | :data |
| `AiAdvisorMessageBlockDaoTest` | 消息块DAO测试 | :data |
| `AiAdvisorChatViewModelTest` | 流式ViewModel测试 | :presentation |
...
class SendAdvisorMessageStreamingUseCaseTest {
    ...
    @Test
    fun `流式响应正常完成时应更新Block状态为SUCCESS`() = runTest {
        // Given
...
### 10.1 Phase 1: 数据层基础（预计2天）

| 任务ID | 任务描述 | 优先级 | 预计工时 |
|--------|---------|--------|---------|
| T1.1 | 创建AiStreamChunk密封类 | P0 | 1h |
| T1.2 | 创建MessageBlockType/MessageBlockStatus枚举 | P0 | 0.5h |
| T1.3 | 创建AiAdvisorMessageBlock模型 | P0 | 1h |
| T1.4 | 创建StreamingState密封类 | P0 | 0.5h |
| T1.5 | 创建AiAdvisorMessageBlockEntity | P0 | 1h |
| T1.6 | 创建AiAdvisorMessageBlockDao | P0 | 1h |
| T1.7 | 编写MIGRATION_13_14迁移脚本 | P0 | 2h |
| T1.8 | 更新AppDatabase（版本升级+新DAO） | P0 | 1h |
| T1.9 | 编写Migration13To14Test | P0 | 2h |

### 10.2 Phase 2: SSE流式实现（预计2天）

| 任务ID | 任务描述 | 优先级 | 预计工时 |
|--------|---------|--------|---------|
| T2.1 | 创建SseStreamReader | P0 | 4h |
| T2.2 | 创建SseFallbackException | P0 | 0.5h |
| T2.3 | 创建BlockUpdateManager | P0 | 3h |
| T2.4 | 扩展AiRepository接口（添加generateTextStream） | P0 | 1h |
| T2.5 | 实现AiRepositoryImpl.generateTextStream | P0 | 2h |
| T2.6 | 扩展AiAdvisorRepository接口（Block相关） | P0 | 1h |
| T2.7 | 实现AiAdvisorRepositoryImpl Block方法 | P0 | 2h |
| T2.8 | 编写SseStreamReaderTest | P1 | 2h |
| T2.9 | 编写BlockUpdateManagerTest | P1 | 2h |

### 10.3 Phase 3: UseCase层（预计1天）

| 任务ID | 任务描述 | 优先级 | 预计工时 |
|--------|---------|--------|---------|
| T3.1 | 创建SendAdvisorMessageStreamingUseCase | P0 | 4h |
| T3.2 | 编写SendAdvisorMessageStreamingUseCaseTest | P0 | 3h |
| T3.3 | 更新AiAdvisorModule（流式DI配置） | P0 | 1h |

### 10.4 Phase 4: UI层（预计2天）

| 任务ID | 任务描述 | 优先级 | 预计工时 |
|--------|---------|--------|---------|
| T4.1 | 创建StreamingMessageBubble组件 | P0 | 3h |
| T4.2 | 创建ThinkingSection组件 | P0 | 3h |
| T4.3 | 创建StopGenerationButton组件 | P1 | 1h |
| T4.4 | 扩展AiAdvisorChatViewModel（流式状态） | P0 | 3h |
| T4.5 | 更新AiAdvisorChatScreen（集成流式组件） | P0 | 3h |
| T4.6 | 编写AiAdvisorChatViewModelTest | P1 | 2h |

### 10.5 Phase 5: 集成测试（预计1天）

| 任务ID | 任务描述 | 优先级 | 预计工时 |
|--------|---------|--------|---------|
| T5.1 | 端到端流式对话测试 | P0 | 3h |
| T5.2 | 思考过程展示测试 | P0 | 2h |
| T5.3 | 停止生成功能测试 | P1 | 1h |
| T5.4 | 降级策略测试 | P2 | 2h |
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| SSE连接不稳定 | 用户体验差 | 实现降级策略，自动切换非流式模式 |
| 数据库迁移失败 | 数据丢失 | 完整的迁移测试，备份机制 |
| 思考过程解析失败 | 功能不完整 | 多模型兼容性测试，fallback处理 |
| 内存占用过高 | 应用卡顿 | 智能节流，及时清理资源 |
...
| 版本 | 日期 | 作者 | 变更内容 |
|------|------|------|---------|
| 1.0 | 2026-01-04 | AI Assistant | 初始版本 |
| 1.1 | 2026-01-04 | AI Assistant | 根据DR-00028审查报告修复：补充SendStatus.CANCELLED、Repository方法、ErrorSection组件、依赖说明 |

---

## 13. 参考文档

- [PRD-00028 AI军师流式对话升级需求](../PRD/PRD-00028-AI军师流式对话升级需求.md)
- [TDD-00028 AI军师流式对话升级技术设计](../TDD/TDD-00028-AI军师流式对话升级技术设计.md)
- [FD-00026 AI军师对话功能设计](./FD-00026-AI军师对话功能设计.md)
- [RESEARCH-00003 Cherry项目架构对比分析报告](../RE/RESEARCH-00003-Cherry项目架构对比分析报告.md)
- [RESEARCH-00004 Cherry项目AI对话实现深度分析报告](../RE/RESEARCH-00004-Cherry项目AI对话实现深度分析报告.md)
