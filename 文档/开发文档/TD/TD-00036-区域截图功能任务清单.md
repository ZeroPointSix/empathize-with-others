...
**集成测试执行记录（MuMu）**:
- 前置：`adb connect 192.0.2.1:7555`
- 安装：`adb -s 192.0.2.1:7555 install -r app/build/outputs/apk/debug/app-debug.apk`、`adb -s 192.0.2.1:7555 install -r app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk`
- 执行：`adb -s 192.0.2.1:7555 shell am instrument -w -r -e class com.empathy.ai.domain.service.FloatingWindowServiceScreenshotTest com.empathy.ai.test/androidx.test.runner.AndroidJUnitRunner` ✅

**Gradle connected（仅 MuMu）**:
- `ANDROID_SERIAL=192.0.2.1:7555 ./gradlew.bat :app:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.empathy.ai.domain.service.FloatingWindowServiceScreenshotTest` ✅
  - PowerShell 兼容写法：`$env:ANDROID_SERIAL='192.0.2.1:7555'; ./gradlew.bat :app:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=com.empathy.ai.domain.service.FloatingWindowServiceScreenshotTest"` ✅

**回归复测（2026-01-13）**:
- `./gradlew.bat :app:testDebugUnitTest` ✅（报告：`app/build/reports/tests/testDebugUnitTest/index.html`）
- `adb connect 192.0.2.1:7555` ✅
- `./gradlew.bat :app:assembleDebug` ✅ + `adb -s 192.0.2.1:7555 install -r app/build/outputs/apk/debug/app-debug.apk` ✅
- `PowerShell: $env:ANDROID_SERIAL='192.0.2.1:7555'; ./gradlew.bat :app:connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=com.empathy.ai.domain.service.FloatingWindowServiceScreenshotTest"` ✅
...
**设备说明（MuMu）**:
- 若 `adb connect 192.0.2.1:7555` 返回 `10061`（连接被拒绝），通常表示 MuMu 未启动或端口未开放，此时 `connectedDebugAndroidTest` / 安装 APK 将无法执行。
