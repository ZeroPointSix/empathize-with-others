...
| 文档类型 | 文档编号 | 文档名称 |
|----------|----------|----------|
| 功能设计 | FD-00027 | 项目修复功能设计 |
| 需求文档 | PRD-00027 | 项目修复需求 |
| 架构审查 | MA-ARCH-0001 | Data层架构审查报告 |
| 架构审查 | - | domain-layer-architecture-review |
| 架构审查 | - | Presentation架构审查报告 |
| 架构审查 | - | APP层架构审查报告 |
| 审查报告 | DR-00027 | FD-00027文档审查报告 |
...
基于MA文档分析结果，系统性地解决项目在代码质量、架构设计和测试覆盖率方面存在的问题。
...
|------|----------|----------|
| :app | 构建配置、DI模块 | 构建系统 |
| :data | Repository实现、Parser重构 | 数据层 |
| :presentation | ViewModel清理、组件目录优化 | 表现层 |
| :domain | UseCase测试、Service拆分 | 领域层 |
...
**根因**：app模块定义了`dev`构建变体，但presentation和data模块没有对应变体。

**影响**：
- `testDevUnitTest`任务无法执行
- IDE中选择dev变体会失败
...
├── PolishDraftUseCaseTest.kt       # 草稿润色测试
├── GenerateReplyUseCaseTest.kt     # 生成回复测试
├── CheckDraftUseCaseTest.kt        # 安全检查测试
├── RefinementUseCaseTest.kt        # 细化优化测试
├── SummarizeDailyConversationsUseCaseTest.kt  # 每日总结测试
├── ManualSummaryUseCaseTest.kt     # 手动总结测试
├── SaveProfileUseCaseTest.kt       # 联系人保存测试
├── GetContactUseCaseTest.kt        # 联系人获取测试
├── SaveBrainTagUseCaseTest.kt      # 标签保存测试
└── EditFactUseCaseTest.kt          # 事实编辑测试
```
...
#### 3.1.3 PolishDraftUseCase测试实现

```kotlin
/**
 * PolishDraftUseCase单元测试
 *
 * 测试覆盖：
 * 1. 正常流程测试
 * 2. 前置检查测试
...
class PolishDraftUseCaseTest : BaseUseCaseTest() {
    ...
    fun `should return polished draft when all conditions are met`() = runTest {
        ...
        coEvery { promptBuilder.buildPolishPrompt(any(), any()) } returns "系统提示词"
    ...
    fun `should return failure when draft exceeds max length`() = runTest {
        // Given
        val contactId = "test-contact-id"
        val draft = "a".repeat(10001) // 超过最大长度
    ...
    fun `should handle special characters correctly`() = runTest {
        ...
        val contact = createTestContactProfile(id = contactId)
        val provider = createTestAiProvider()
        val expectedResult = PolishResult(polishedText = "润色后的文字", suggestions = emptyList())
...
│                    FallbackHandler                           │
├─────────────────────────────────────────────────────────────┤
│ + handleFallback<T>(context: FallbackContext<T>):           │
│                     FallbackResult<T>                        │
└─────────────────────────────────────────────────────────────┘
                              △
                              │ implements
...
│ - logger: Logger                                             │
├─────────────────────────────────────────────────────────────┤
│ + handleFallback<T>(context): FallbackResult<T>             │
│ - tryStrategy<T>(strategy, context): FallbackResult<T>      │
│ - extractText<T>(context): FallbackResult<T>                │
│ - mapFields<T>(context): FallbackResult<T>                  │
│ - useDefaultValue<T>(context): FallbackResult<T>            │
│ - partialParse<T>(context): FallbackResult<T>               │
└─────────────────────────────────────────────────────────────┘
```
...
class DefaultFallbackHandler @Inject constructor(
    ...
    private fun <T> tryStrategy(
        ...
        return try {
            ...
            logger.w(TAG, "策略 $strategy 执行异常", e)
    ...
    private fun <T> extractText(context: FallbackContext<T>): FallbackResult<T> {
        ...
        return when (context.targetType) {
            ...
            ReplyResult::class.java -> {
                ...
                UnsupportedOperationException("不支持的目标类型: ${context.targetType.simpleName}")
            )
        }
    }
...
class UnifiedAiResponseParser @Inject constructor(
    ...
    private fun <T> parseWithFallback(
        ...
        return try {
            ...
            logger.w(TAG, "标准解析失败，尝试降级处理: ${e.message}")
...
### 3.4 AiRepositoryImpl拆分技术设计

#### 3.4.1 拆分后文件结构

```
data/src/main/kotlin/com/empathy/ai/data/repository/ai/
├── AiRepositoryImpl.kt           # 主Repository实现（~250行）
├── AiRequestBuilder.kt           # AI请求构建器（~150行）
├── AiResponseHandler.kt          # AI响应处理器（~200行）
└── AiProviderAdapter.kt          # AI服务商适配器（~150行）
```

#### 3.4.2 AiRequestBuilder实现
...
class AiRequestBuilder @Inject constructor(
    ...
    fun buildChatRequest(
        ...
        logger.d(TAG, "构建聊天请求，模型: ${provider.model}")
    ...
    fun buildPolishRequest(
        ...
        logger.d(TAG, "构建润色请求，历史上下文数量: ${historyContext.size}")
...
class AiProviderAdapter @Inject constructor(
    ...
    suspend fun executeRequest(
        ...
        logger.d(TAG, "执行请求，服务商: ${provider.name}, URL: ${provider.baseUrl}")
    ...
    private suspend fun <T> executeWithRetry(
        ...
        repeat(MAX_RETRIES) { attempt ->
            try {
                ...
                lastException = e
                logger.w(TAG, "第${attempt + 1}次尝试失败: ${e.message}")
            }
        ...
        logger.e(TAG, "所有重试均失败", lastException)
...
### 3.5 重复ViewModel合并技术设计

#### 3.5.1 问题分析

当前存在两个重复的Debug版本ViewModel：
- `ContactDetailTabViewModelDebug.kt`
- `ContactDetailTabViewModelWithDebug.kt`

#### 3.5.2 合并方案

使用`@VisibleForTesting`注解暴露测试方法，合并为单一ViewModel：
...
### 4.1 PrivacyEngine拆分技术设计

#### 4.1.1 拆分后结构

```
domain/src/main/kotlin/com/empathy/ai/domain/service/privacy/
├── SensitiveDataDetector.kt    # 敏感信息检测器
├── PrivacyMasker.kt            # 隐私数据脱敏器
└── PrivacyEngine.kt            # 门面类（保持向后兼容）
```

#### 4.1.2 SensitiveDataDetector实现
...
#### 4.1.4 PrivacyEngine门面类

```kotlin
/**
 * 隐私引擎（门面类）
 *
 * 职责：提供统一的隐私处理入口，保持向后兼容
 */
...
class AnalyzeChatUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
    ...
    ): Result<AnalysisResult> = safeCall(logger, TAG, "分析聊天失败") {
        // 获取联系人
        ...
        val provider = aiProviderRepository.getDefaultProvider()
            .logOnError(logger, TAG, "获取AI服务商失败")
            .getOrThrow()
...
private fun createTestContactProfile(
    id: String = "test-id",
    name: String = "测试联系人",
    contextDepth: Int = 5,  // 必须 > 0
    lastInteractionDate: String? = null,  // 使用null
    originalGoal: String? = null  // 使用null
): ContactProfile {
    return ContactProfile(
        id = id,
        name = name,
...
| 阶段 | 内容 | 工时 | 依赖 |
|------|------|------|------|
| Phase 1 | P0构建配置修复 | 0.5天 | 无 |
| Phase 2 | 测试修复 | 0.5天 | Phase 1 |
| Phase 3 | FallbackHandler实现 | 1天 | Phase 2 |
| Phase 4 | Parser接口重构 | 2天 | Phase 3 |
| Phase 5 | AiRepositoryImpl拆分 | 2天 | Phase 4 |
| Phase 6 | ViewModel合并 | 0.5天 | Phase 2 |
| Phase 7 | 组件目录优化 | 1天 | Phase 2 |
| Phase 8 | UseCase测试覆盖 | 5天 | Phase 5 |
| Phase 9 | P2问题修复 | 3天 | Phase 8 |
| Phase 10 | 验证和文档 | 1天 | Phase 9 |
