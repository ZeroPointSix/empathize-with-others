...
| Domain | `EditFactUseCase.kt` | 单元测试 |
| Domain | `EditConversationUseCase.kt` | 单元测试 |
| Domain | `EditSummaryUseCase.kt` | 单元测试 |
| Domain | `EditContactInfoUseCase.kt` | 单元测试 |
| Domain | `ContentValidator.kt` | 单元测试 |
...
#### TC-012：事实编辑校验失败返回ValidationError

```kotlin
@Test
fun `事实类型为空返回验证错误`() = runTest {
    val result = useCase("contact-1", "fact-1", "", "value")

    assertTrue(result.isSuccess)
    assertTrue(result.getOrNull() is EditResult.ValidationError)
}
```

#### TC-013：对话记录不存在返回NotFound

```kotlin
@Test
fun `记录不存在返回NotFound`() = runTest {
    coEvery { conversationRepository.getById(1L) } returns null

    val result = useCase(1L, "${IdentityPrefixHelper.PREFIX_USER}你好")
...
#### TC-014：总结更新成功返回Success

```kotlin
@Test
fun `更新成功返回Success`() = runTest {
    val summary = createSummary(content = "原总结")
    coEvery { dailySummaryRepository.getById(1L) } returns summary
    coEvery { dailySummaryRepository.updateContent(any(), any(), any(), any()) } returns 1

    val result = useCase(1L, "新总结")
...
#### TC-015：联系人姓名为空返回ValidationError

```kotlin
@Test
fun `姓名为空返回验证错误`() = runTest {
    val result = useCase.editName("contact-1", " ")
...
#### TC-016：ContentValidator 对事实内容超长拦截

```kotlin
@Test
fun `事实内容超长返回错误`() {
    val longValue = "a".repeat(ContentValidator.MAX_FACT_VALUE_LENGTH + 1)

    val result = validator.validateFactValue(longValue)

    assertTrue(result is ContentValidator.ValidationResult.Error)
}
```
...
| 2026-01-18 | v1.1 | 补充Domain层编辑用例与ContentValidator测试用例 | Codex |
