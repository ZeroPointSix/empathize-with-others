...
## 环境
- 项目：PRD36-picture
- 设备：MuMu 模拟器（127.0.0.1:7555）、OPPO 真机（QCUKF6DUW46XKVU8）
...
2. 启动与截图验证
   - `adb -s 127.0.0.1:7555 shell am start -n com.empathy.ai/com.empathy.ai.ui.MainActivity`
   - `adb -s 127.0.0.1:7555 exec-out screencap -p > ...\mumu-xxx.png`
   - 观察：App 内悬浮球未显示；桌面截图曾观察到悬浮球存在。

3. 权限检查
   - `adb -s 127.0.0.1:7555 shell cmd appops get com.empathy.ai SYSTEM_ALERT_WINDOW`
   - 结果：SYSTEM_ALERT_WINDOW 允许。
   - `adb -s 127.0.0.1:7555 shell cmd appops get com.empathy.ai POST_NOTIFICATION`
   - 结果：默认允许或无记录。

4. 服务状态与日志
   - `adb -s 127.0.0.1:7555 shell dumpsys activity services | rg "FloatingWindowService|com.empathy.ai"`
   - 结果：服务处于运行中。
   - `adb -s 127.0.0.1:7555 logcat -d -v time | rg "FloatingWindowService|AndroidFloatingWindowManager"`
   - 结果：日志显示“悬浮球已显示”等，但 UI 未在 App 内出现。

5. 窗口层级与显示屏分析
   - `adb -s 127.0.0.1:7555 shell dumpsys window windows`
   - 现象：
     - App 窗口在 displayId=8。
